...
Context.prototype = {
    ...
    setCurrentUser: function (user) {
        ...
        if (!this._currentUser.isUnknownUser()) {
            ...
            if (last_login) {
                ...
                var acceptableTime = loginMoment.add(
                    last_login_buffer,
                    "seconds"
                );
            ...
            analytics.setCustomVariable(
                analytics.customVariables.HAS_ELEVATED_RIGHTS,
                this._currentUser.can(Permissions.EDIT_EXTRACT)
            );

            if (this._currentUser.get("post_count") >= 1) {
                analytics.setCustomVariable(
                    analytics.customVariables.HAS_POSTED_BEFORE,
                    true
                );
            } else {
                analytics.setCustomVariable(
            ...
            if (
                this._currentUser.get("first_visit") !== null &&
                this._currentUser.get("last_visit") !== null
            ) {
                //Note:  moment always rounds DOWN
                days_since_first_visit = Moment(
                    this._currentUser.get("last_visit")
                ).diff(this._currentUser.get("first_visit"), "days");
                if (days_since_first_visit >= 1) {
                    analytics.setCustomVariable(
                        analytics.customVariables.IS_ON_RETURN_VISIT,
                        true
                    );
                } else {
                    analytics.setCustomVariable(
            ...
            collectionManager
                ...
                .then(function (localRoles) {
                    var logUserSubscriptionStatus = function (localRoles) {
                        analytics.setCustomVariable(
                            analytics.customVariables.IS_DISCUSSION_MEMBER,
                            localRoles.isUserSubscribedToDiscussion()
                        );
                    };
                    localRoles.listenTo(
                        localRoles,
                        "update",
                        logUserSubscriptionStatus
                    );
    ...
    getMessageViewStyleDefById: function (messageViewStyleId) {
        return _.find(this.AVAILABLE_MESSAGE_VIEW_STYLES, function (
            messageViewStyle
        ) {
    ...
    getApiV2DiscussionUrl: function (url) {
        ...
        return this.getApiV2Url(
            Types.DISCUSSION + "/" + this.getDiscussionId() + url
        );
    },
    /**
    ...
    DEPRECATEDgetMessageListConfigFromStorage: function () {
        var messageListConfig =
            JSON.parse(localStorage.getItem("messageListConfig")) || {};
    ...
    DEPRECATEDsetMessageListConfigToStorage: function (messageListConfig) {
        localStorage.setItem(
            "messageListConfig",
            JSON.stringify(messageListConfig)
        );
        return messageListConfig;
    },
    /**
    ...
    openTargetInPopOver: function (evt) {
        ...
        if (!target_url) {
            console.log(
                "context::openTargetInPopOver: no href attribute given"
            );
            return false;
        }
        ...
        if (evt && evt.currentTarget) {
            ...
            if ($(evt.currentTarget).attr("data-popover-height"))
                popover_height = $(evt.currentTarget).attr(
                    "data-popover-height"
                );
            if ($(evt.currentTarget).attr("data-popover-scrolling"))
                popover_scrolling = $(evt.currentTarget).attr(
                    "data-popover-scrolling"
                );
        }
        ...
        var iframe =
            ...
            popover_scrolling +
            '" frametransparency="1" src="' +
            target_url +
            '" style="display: block;"></iframe>'; // "display: block;" is added in order to avoid an empty space at the bottom of the iframe
        ...
        var triggerHover = function (evt) {
            ...
            popover.css(
                "top",
                evt.pageY - parseInt(popover_height) - 14 + "px"
            );
    ...
    openTargetInModal: function (evt, onDestroyCallback, options) {
        ...
        if (
            evt &&
            evt.currentTarget &&
            $(evt.currentTarget).attr("data-modal-title")
        )
        ...
        if (
            evt &&
            evt.currentTarget &&
            $(evt.currentTarget).attr("data-modal-resize-on-load")
        )
            resizeIframeOnLoad =
                $(evt.currentTarget).attr("data-modal-resize-on-load") !=
                    false &&
        ...
        if (
            evt &&
            evt.currentTarget &&
            $(evt.currentTarget).attr("data-modal-resizable")
        )
            resizable =
                $(evt.currentTarget).attr("data-modal-resizable") != false &&
                $(evt.currentTarget).attr("data-modal-resizable") != "false";
        ...
        if (!resizable) className += " popin-fixed-size";
        ...
        window.resizeIframe = function (iframe, retry) {
            if (!iframe) iframe = $(".iframe-popin iframe").get(0);
            if (!iframe) return;
            ...
            if (targetHeight > 10) {
                ...
                if (targetWidth > 10) {
                    ...
                    animatingProperties.width =
                        targetWidth + addPixelsToCompensateMargins + "px";
                }

                $(iframe).animate(animatingProperties, {
                    complete: function () {
    ...
    getCurrentSynthesisDraftPromise: function () {
        ...
        if (this.currentSynthesisDraftPromise === null) {
            ...
            this.currentSynthesisDraftPromise = collectionManager
                .getAllSynthesisCollectionPromise()
                .then(function (syntheses) {
    ...
    getPostIdFromAnnotation: function (annotation) {
        ...
        var messageId = span
            .closest('[id^="' + this.ANNOTATOR_MESSAGE_BODY_ID_PREFIX + '"]')
            .attr("id");
    ...
    saveCurrentAnnotationAsExtract: function () {
        if (this.getCurrentUser().can(Permissions.ADD_EXTRACT)) {
            ...
            alert(
                "Error: You don't have the permission to save this annotation as an extract."
            );
    ...
    showDragbox: function (ev, text, newExtract) {
      ...
      */
            if ("setDragImage" in ev.dataTransfer) {
                // Internet Explorer and Edge are currently the only browsers known to not support dataTransfer.setDragImage(), and to recognize only "text" instead of "text/plain".
    ...
    getNiceDateTime: function (date, precise, with_time, forbid_future) {
        ...
        if (forbid_future) {
            // server time may be ahead of us of some minutes. In this case, say it was now
            var now = Moment();
        ...
        if (momentDate) {
            if (precise == true) {
                if (with_time == true) return momentDate.format("LLLL");
                else return momentDate.format("LL");
            }
    ...
    getNiceDate: function (date, precise, forbid_future) {
        if (precise === undefined) precise = true;
    ...
    showTargetBySegment: function (segment) {
        ...
        switch (target["@type"]) {
            ...
            default:
                // This will treat:
                // ['Email', 'Post', 'LocalPost', 'SynthesisPost', 'ImportedPost', 'WidgetPost', 'IdeaProposalPost']

                var selector = this.format(
                    '[data-annotation-id="{0}"]',
                    segment.id
                );

                IdeaLoom.message_vent.trigger(
   ...
   return uuid;
    ...
    onAjaxError: function (ev, jqxhr, settings, exception) {
        if (jqxhr.handled) return;

        // ignore Ajax errors which come from outside (sub-)domains. This is useful for oembed related errors
        var getHostnameFromUrl = function (data) {
            // hostname examples: "localhost", "localhost:4321"
        ...
        if (
            settings &&
            "url" in settings &&
            window.location.hostname != getHostnameFromUrl(settings.url)
        ) {
            console.log(
                "ignoring Ajax error from outside domain: ",
                getHostnameFromUrl(settings.url)
            );
    ...
    canUseExpertInterface: function () {
        ...
        if (
            user.can(Permissions.ADD_EXTRACT) ||
    ...
    manageLastCurrentUser: function () {
        ...
        if (window.localStorage.getItem("lastCurrentUser")) {
            lastCurrentUserId = window.localStorage
                .getItem("lastCurrentUser")
                .split("/")[1];
        }
        ...
        if (connectedUserId) {
            ...
            if (connectedUserId != lastCurrentUserId) {
                ...
                console.info(
                    "Clearing preferences since the logged-in user changed, or there was no previous logged-in user"
                );
    ...
    getPermissionTokenPromise: function (permission_sets, user_ids) {
        ...
        var url =
            this.getApiV2DiscussionUrl("perm_token") +
            "?" +
            permissions +
            "&" +
            _.map(user_ids, function (p) {
                return "user_id=" + p;
            }).join("&");
    ...
    deanonymizationCifInUrl: function (url, callback) {
        ...
        var url_base =
            ...
            Ctx.getDiscussionId();
        Promise.join(
            this.getPermissionTokenPromise(
                [cif_perms, user_perms],
                ...
                callback(
                    urlTemplate({
                        url: encodeURIComponent(
                            url_base + "/jsonld?token=" + cif_token
                        ),
                        user_url: encodeURIComponent(
                            url_base + "/private_jsonld?token=" + user_token
                        ),
                        lang: assembl_locale,
                        user_ids: encodeURIComponent(
                            token_data["user_ids"].join(",")
                        ),
                    })
                );
            }
    ...
    getTranslationLocaleToLanguageNameCache: function () {
        ...
        this._translationLocaleToLangNameCache = this.getJsonFromScriptTag(
    ...
    localesAsSortedList: function () {
        if (this._localesAsSortedList === undefined) {
            ...
            var localeList = _.mapObject(
                this.getTranslationLocaleToLanguageNameCache(),
                function (name, loc) {
                    return [loc, name];
                }
            );
    ...
    getTranslationServiceData: function () {
        if (this.isApplicationUnderTest()) {
            ...
            this._translationServiceCache = this.getJsonFromScriptTag(
