...
_.extend(AnalyticsDispatcher.prototype, {
    ...
    notify: function (methodName, args, check_cb) {
        ...
        _.each(this._observers, function (observer) {
            ...
            if (observer.debug) {
                console.log(
                    "Invoking method " + methodName + " with argument(s)",
                    args,
                    "on observer",
                    observer
                );
    ...
    setCustomVariable: function (variableDefinition, value) {
        if (
            _.indexOf(_.values(this.customVariables), variableDefinition) === -1
        ) {
    ...
    getInstance: function () {
        if (!_analytics) {
            ...
            if (globalAnalytics.piwik && globalAnalytics.piwik.isActive) {
                ...
                globalAnalytics.google &&
                globalAnalytics.google.isActive
            ) {
                if (_analytics.debug) {
