...
var attachmentPurposeTypes = {
    ...
    EMBED_ATTACHMENT: {
        id: "EMBED_ATTACHMENT",
        label: i18n.gettext(
            "Show the following preview at the end of your message"
        ),
    },
    /**
     * Future:
...
class AttachmentModel extends Base.Model.extend({
    ...
    parse(rawModel) {
        switch (rawModel.document["@type"]) {
            case Types.DOCUMENT:
                rawModel.document = new Document.DocumentModel(
                    rawModel.document,
                    { parse: true }
                );
                break;
            case Types.FILE:
                rawModel.document = new Document.FileModel(rawModel.document, {
                    parse: true,
                });
                break;
            default:
                return new Error(
                    "The document model does not have a @type associated!" +
                        rawModel.document
                );
        }
        return rawModel;
    ...
    save(attrs, options) {
        ...
        var that = this;
        if (
            this.get("attachmentPurpose") !==
            attachmentPurposeTypes.DO_NOT_USE.id
        ) {
    ...
    sync(method, model, options) {
        switch (method) {
            ...
            case "create":
                ...
                var objectAttachedToBaseType = Types.getBaseType(
                    this.get("objectAttachedToModel").get("@type")
                );
    ...
    destroy(options) {
        ...
        return d.destroy({
            ...
            error: function (model, response) {
                console.warn(
                    "Could NOT destroy document model id " +
                        d.id +
                        "\nContinuing to destroy attachment id " +
                        this.id
                );
                return Base.Model.prototype.destroy.call(that);
            },
...
class AttachmentCollection extends Base.Collection {
    ...
    url() {
        return (
            this.objectAttachedToModel.urlRoot() +
            "/" +
            this.objectAttachedToModel.getNumericId() +
            "/attachments"
        );
    }
    ...
    initialize(models, options) {
        if (!options.objectAttachedToModel) {
            throw new Error(
                "objectAttachedToModel must be provided to calculate url"
            );
        } else {
   ...
   */
    addValidation(models) {
        ...
        if (!this.isCountLimitCorrect(models)) {
            correctedNumberModelsCollection = this.getCorrectCountedCollection(
                models
            );
            ...
            correctedNumberModelsCollection =
                correctedNumberModelsCollection.collection;
        }
        ...
        _.each(correctedNumberModelsCollection, function (model) {
            if (!that.isTypeLimitCorrect(model)) {
                ...
                Growl.showBottomGrowl(
                    ...
                    i18n.gettext(
                        "Sorry, only an image type is supported in this upload. Please try again."
                    )
                );
            }
        });
