...
class BaseModel extends Backbone.Model.extend({
    ...
    url() {
        ...
        return (
            base.replace(/([^\/])$/, "$1/") +
            encodeURIComponent(this.getNumericId())
        );
    }
        ...
        if (resp["ok"] === true && id !== undefined) {
            if (this.collection !== undefined) {
                ...
                if (existing === null || existing === undefined) {
                    ...
                    console.log(
                        "base:parse(): this should not happen, but no harm..."
                    );
                }
            }
        }
...
class BaseCollection extends Backbone.Collection {
    ...
    fetchFromScriptTag(id, parse) {
        ...
        return Promise.delay(1).then(function () {
            ...
            if (!script) {
                throw new Error(
                    Ctx.format("Script tag #{0} doesn't exist", id)
                );
            }
    ...
    updateFromSocket(item) {
        ...
        if (item["@tombstone"]) {
            if (model) {
                if (debug) {
                    console.log(
                        "updateFromSocket(): Removing from socket (tombstoned):",
                        model.get("@type"),
                        model.id
                    );
                }
                ...
                if (debug) {
                    console.log(
                        "updateFromSocket(): Ignoring tombstone not in collection: ",
                        item["@type"],
                        item["@id"],
                        item
                    );
            ...
            if (debug) {
                console.log(
                    "updateFromSocket(): Merging from socket:",
                    item["@type"],
                    item["@id"],
                    item
                );
...
class RelationsCollection extends BaseCollection {
    ...
    remove(models, options) {
        ...
        if (
            models !== undefined &&
            (options === undefined || options.previousModels === undefined)
        ) {
            var that = this;
            var singular = !_.isArray(models);
            var modelsArray = singular ? [models] : models;
