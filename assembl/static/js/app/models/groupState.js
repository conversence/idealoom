...
class GroupStateModel extends Base.Model.extend({
    ...
    toJSON(options) {
        ...
        if (
            json.currentIdea !== null &&
            json.currentIdea instanceof Idea.Model
        ) {
...
class GroupStates extends Base.Collection.extend({
    ...
    validate() {
        ...
        if (invalid.length) {
            console.warn(
                "GroupState.Collection: removing " +
                    invalid.length +
                    " invalid groupStates from " +
                    this.length +
                    " groupStates."
            );
            this.remove(invalid);
            console.warn(
                "GroupState.Collection: after removal, number of remaining valid groupStates: " +
                    this.length
            );
        }
