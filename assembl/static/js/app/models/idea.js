...
class IdeaModel extends Base.Model.extend({
    ...
    initialize(obj) {
        ...
        this.set(
            "hasCheckbox",
            Ctx.getCurrentUser().can(Permissions.EDIT_SYNTHESIS)
        );
    ...
    parse(resp, options) {
        ...
        if (resp.ok !== true) {
            ...
            if (resp.shortTitle !== undefined) {
                resp.shortTitle = new LangString.Model(resp.shortTitle, {
                    parse: true,
                });
            }
            if (resp.longTitle !== undefined) {
                resp.longTitle = new LangString.Model(resp.longTitle, {
                    parse: true,
                });
            }
            if (resp.definition !== undefined) {
                resp.definition = new LangString.Model(resp.definition, {
                    parse: true,
                });
            }
            if (resp.attachments !== undefined) {
                resp.attachments = new Attachment.ValidationAttachmentCollection(
                    resp.attachments,
                    {
                        parse: true,
    ...
    isDescendantOf(idea) {
        ...
        return parentId === null
            ? false
            : this.getParent().isDescendantOf(idea);
    }
    ...
    getPossibleCombinedSubtypes(parentLink) {
        ...
        if (parentLink.get("target") != this.id) {
            ...
            var preferences = Ctx.getPreferences();
            var parent = this.collection.findWhere({
                "@id": parentLink.get("source"),
            });
            if (parent) {
                var parentSubtype = parent.get("subtype");
                var typologyParentInfo =
                    preferences.idea_typology.ideas[parentSubtype];
                if (typologyParentInfo && typologyParentInfo.rules) {
                    var result = [];
                    _.mapObject(typologyParentInfo.rules, function (
                        objectTypes,
                        linkType
                    ) {
    ...
    combinedTypeNames(parentLink, lang) {
        return this.combinedPresentationOf(
            this.getCombinedSubtypes(parentLink),
            lang
        );
    }
        ...
        return this.collection.collectionManager
            ...
            .then(function (allExtractsCollection) {
                return Promise.resolve(
                    allExtractsCollection.filter((extract) =>
    ...
    set(key, val, options) {
        if (typeof key === "object") {
            ...
            if (
                ...
                var id = attrs["@id"];
                var links = this.collection.collectionManager._allIdeaLinksCollection.where(
                    { target: id }
                );
                if (links.length > 0) {
