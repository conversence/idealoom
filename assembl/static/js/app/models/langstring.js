...
var LocaleUtils = {
    ...
    localeCompatibility: function (locale1, locale2) {
        ...
        // Google special case
        if (locale1 == "zh") locale1 = "zh_Hans";
        if (locale2 == "zh") locale2 = "zh_Hans";
    ...
    stripCountry: function (locale) {
        ...
        if (
            ...
            locale_parts[locale_parts.length - 1].length == 2
        ) {
            locale_parts.pop();
    ...
    localeAsTranslationService: function (locale) {
        ...
        if (parts.length > 1) {
            return [
                this.localeAsTranslationService(parts[0]),
                this.localeAsTranslationService(parts[1]),
            ].join("-x-mtfrom-");
        }
...
class LangStringEntry extends Base.Model.extend({
    ...
    original() {
        ...
        if (
            ...
            this.collection.langstring !== undefined
        ) {
...
class LangString extends Base.Model.extend({
    ...
    original() {
        ...
        var originals = entries.filter(function (e) {
            return !e.isMachineTranslation();
        });
            ...
            for (
                ...
                for (var i = 0; i < available.length; i++) {
                    ...
                    if (entry.isMachineTranslation() != useTranslations)
                        continue;
                    if (filter_errors && entry.get("error_code")) continue;
                ...
                if (prefCandidates.length) {
                    prefCandidates = _.sortBy(
                        prefCandidates,
                        for_interface
                            ? langPrefs.interface_comparator
                            : langPrefs.comparator
                    );
                    for (i = 0; i < prefCandidates.length; i++) {
                        ...
                        if (!translate_to) {
                            ...
                            commonLenF = function (entry) {
                                return (
                                    LocaleUtils.localeCompatibility(
                                        entry.get("@language"),
                                        translate_to
                                    ) !== false
                                );
