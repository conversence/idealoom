...
class LanguagePreferenceCollection extends Base.Collection.extend({
    ...
    comparator(lp) {
        if (
            LangString.LocaleUtils.stripCountry(lp.get("locale_code")) ==
                Ctx.getLocale() &&
        ...
        return (
            1 + lp.get("source_of_evidence") + lp.get("preferred_order") / 100.0
        );
    }
    ...
    getPreferenceForLocale(locale) {
        if (this.cachePrefByLocale === undefined) {
            console.warn(
                "getPreferenceForLocale was called before getTranslationData"
            );
        ...
        if (
            LangString.LocaleUtils.localeCompatibility(
                this.cacheDefaultTargetLocale,
                locale
            ) !== false
        ) {
        ...
        if (this.cachePrefByLocale === undefined) {
            ...
            for (i = 0; i < this.models.length; i++) {
                ...
                if (pref.get("translate_to_name") !== null) {
                    this.cacheDefaultTargetLocale = pref.get(
                        "translate_to_name"
                    );
                    return this;
                }
        ...
        if (currentUser) {
            ...
            var ops = {
                ...
                error: function (model, resp, options) {
                    console.error(
                        "Failed to save user language preference of " +
                            model +
                            " to the database",
                        resp
                    );
            ...
            if (existingModel) {
                ...
                var langPref = new LanguagePreferenceModel(hash, {
                    collection: this,
                });
                ops.wait = false;
