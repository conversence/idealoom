...
BlockingPublicationStates[PublicationStates.MODERATED_TEXT_NEVER_AVAILABLE] =
    PublicationStates.MODERATED_TEXT_NEVER_AVAILABLE;
BlockingPublicationStates[PublicationStates.DELETED_BY_USER] =
    PublicationStates.DELETED_BY_USER;
BlockingPublicationStates[PublicationStates.DELETED_BY_ADMIN] =
    PublicationStates.DELETED_BY_ADMIN;

var ModeratedPublicationStates = {};
ModeratedPublicationStates[PublicationStates.MODERATED_TEXT_NEVER_AVAILABLE] =
    PublicationStates.MODERATED_TEXT_NEVER_AVAILABLE;
ModeratedPublicationStates[PublicationStates.MODERATED_TEXT_ON_DEMAND] =
    PublicationStates.MODERATED_TEXT_ON_DEMAND;

var DeletedPublicationStates = {};
DeletedPublicationStates[PublicationStates.DELETED_BY_USER] =
    PublicationStates.DELETED_BY_USER;
DeletedPublicationStates[PublicationStates.DELETED_BY_ADMIN] =
    PublicationStates.DELETED_BY_ADMIN;
...
class MessageModel extends Base.Model.extend({
    ...
    parse(rawModel) {
        if (rawModel.attachments !== undefined) {
            rawModel.attachments = new Attachment.Collection(
                rawModel.attachments,
                { parse: true, objectAttachedToModel: this }
            );
        }
        if (rawModel.subject !== undefined) {
            rawModel.subject = new LangString.Model(rawModel.subject, {
                parse: true,
            });
        }
        if (rawModel.body !== undefined) {
            rawModel.body = new LangString.Model(rawModel.body, {
                parse: true,
            });
        }
    ...
    getSubjectNoRe() {
        ...
        if (subjectText) {
            return subjectText.replace(/( *)?(RE) *(:|$) */gim, "");
        } else {
            return subjectText;
        }
    }
    ...
    getAnnotationsPromise() {
        var that = this;
        return this.getExtractsPromise().then(function (extracts) {
            var ret = [];
        ...
        return this.collection.collectionManager
            ...
            .then(function (allExtractsCollection) {
                return Promise.resolve(
                    allExtractsCollection.where({ idPost: that.getId() })
                ).catch(function (e) {
    ...
    getCreatorPromise() {
        ...
        return this.collection.collectionManager
            ...
            .then(function (allUsersCollection) {
                return Promise.resolve(
                    allUsersCollection.getById(that.get("idCreator"))
                ).catch(function (e) {
                    console.error(e.statusText);
                });
            });
    ...
    getModeratorPromise() {
        ...
        return this.collection.collectionManager
            ...
            .then(function (allUsersCollection) {
                return Promise.resolve(
                    allUsersCollection.getById(that.get("moderator"))
                ).catch(function (e) {
                    console.error(e.statusText);
                });
            });
    ...
    sync(method, model, options) {
        ...
        if (method == "patch") {
            // for REST calls of type PATCH, we use APIv2 instead of APIv1
    ...
    destroy(options) {
        ...
        var that = this;
        return Promise.resolve(attachments.destroyAll(options)).then(
            function () {
