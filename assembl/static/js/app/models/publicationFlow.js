...
class publicationFlowModel extends Base.Model.extend({
    ...
    parse(rawModel, options) {
        if (rawModel) {
            rawModel.states = new publicationStateCollection(rawModel.states, {
                parse: true,
                flow: this,
            });
            rawModel.transitions = new publicationTransitionCollection(
                rawModel.transitions,
                { parse: true, flow: this }
            );
            if (rawModel.name) {
                rawModel.name = new LangString.Model(rawModel.name, {
                    parse: true,
                });
            }
        }
...
class publicationStateCollection extends Base.Collection.extend({
    ...
    findByLabel(label) {
        const stateA = this.filter((state) => {
            return state.get("label") == label;
        });
        if (stateA && stateA.length) return stateA[0];
    }
}
