...
class SynthesisModel extends Base.Model.extend({
    ...
    set(key, val, options) {
        ...
        if (
            (key == "ideas" || key.ideas !== undefined) &&
            this.ideasCollection !== undefined
        ) {
    ...
    parse(resp, options) {
        if (resp.ok !== true) {
            if (resp.introduction != null) {
                resp.introduction = new LangString.Model(resp.introduction, {
                    parse: true,
                });
            }
            if (resp.subject != null) {
                resp.subject = new LangString.Model(resp.subject, {
                    parse: true,
                });
            }
            if (resp.conclusion != null) {
                resp.conclusion = new LangString.Model(resp.conclusion, {
                    parse: true,
                });
            }
        }
...
class SynthesisCollection extends Base.Collection.extend({
    ...
    getPublishedSyntheses() {
        return this.filter(function (model) {
            return model.get("published_in_post") != null;
        });
    }
