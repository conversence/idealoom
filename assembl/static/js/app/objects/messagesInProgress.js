...
class messagesInProgress extends MnObject.extend({
    ...
    getMessage(context) {
        ...
        if (
            messages[context] &&
            messages[context]["body"] === "[object Object]"
        ) {
            console.warn(
                "savePartialMessage() Fixing corrupted data in message storage (a past bug causes text representation of objects to be stored in local storage)"
            );
