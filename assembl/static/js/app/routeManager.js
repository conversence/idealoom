...
var trackAnalyticsWithQueryString = function (qs, context) {
    ...
    function doCheck(param, success) {
        ...
        if (_.has(QUERY_STRINGS, k)) {
            ...
            console.warn(
                "[Analytics] Query string " + k + "=" + v + " does nothing"
            );
        }
    }
            ...
            case "notification":
                if (context === "post") {
                    ...
                    analytics.trackEvent(
                        analytics.events.ENTER_POST_VIA_NOTIFICATION
                    );
                } else {
    ...
    if (qs) {
        ...
        console.warn(
            "Ùnable to track event, there are no event tracking query parameters present."
        );
    }
};
...
class routeManager extends Marionette.Object {
    ...
    post(id, qs) {
        ...
        this.restoreViews(undefined, undefined, true).then(function (groups) {
            ...
            var messageList = firstGroup.findViewByType(
            ...
            if (!messageList) {
                if (firstGroup.isSimpleInterface()) {
                    ...
                    throw new Error(
                        "WRITEME:  There was no group with a messagelist available"
                    );
                }
            }
            if (
                !messageList.isViewStyleThreadedType(
                    messageList.currentViewStyle
                )
            ) {
    ...
    user(id, qs) {
        this.restoreViews().then(function () {
            ...
            collectionManager
                ...
                .then(function (agentsCollection) {
   ...
   */
    ...
    widgetInModal(id, arg) {
        ...
        this.restoreViews().then(function (groups) {
            var collectionManager = CollectionManager();
            var widgetPromise = collectionManager
                .getAllWidgetsPromise()
                .then(function (allWidgetsCollection) {
                    return Promise.resolve(allWidgetsCollection.get(id)).catch(
                        function (e) {
    ...
    restoreViews(from_home, url_structure, skip_group_state) {
        ...
        if (!Ctx.isSmallScreen()) {
            InfobarsCollection.prototype.createCollection(
                collectionManager,
                function (infobarsCollection) {
                    IdeaLoom.rootView.showChildView(
                        "infobarRegion",
                        new InfobarsViews.InfobarsView({
                            collection: infobarsCollection,
                        })
                    );
                }
            );
         ...
         * */
        var groupSpecsP = this.collectionManager.getGroupSpecsCollectionPromise(
            ViewsFactory,
            url_structure,
            skip_group_state
        );
        ...
        return groupSpecsP.then(function (groupSpecs) {
            ...
            if (
                from_home &&
                !lastSave &&
                (currentUser.isUnknownUser() ||
                    currentUser.get("is_first_visit"))
            ) {
                Promise.join(
           ...
           usable, or if it wasn't initialized before */
                    ...
                    if (navigableGroupSpec) {
                        setTimeout(function () {
                            var groupContent = groupsView.children.findByModel(
                                navigableGroupSpec
                            );
    ...
    userHaveAccess() {
        ...
        switch (route) {
            ...
            case "notifications":
            case "partners":
                access = !Ctx.getCurrentUser().can(Permissions.ADMIN_DISCUSSION)
                    ? false
                    : true;
                break;

            default:
                access = !Ctx.getCurrentUser().can(Permissions.READ)
                    ? false
                    : true;
                break;
        }
        ...
        if (!access) {
            var authorization = new Authorization({
                error: 401,
                message: i18n.gettext(
                    "Your level of permissions do not allow you to see the rest of this content"
                ),
            });
            IdeaLoom.rootView.showChildView("groupContainer", authorization);
        }
