...
var getElementViewportOffset = function (el, scrollableViewport) {
    ...
    if (debugScrollUtils) {
        console.log(
            "scrollUtils::getElementViewportOffset(): scrollableViewportWindowOffset: ",
            scrollableViewportWindowOffset,
            "elWindowOffset: ",
            elWindowOffset,
            el.offset(),
            el,
    ...
    if (el.offset().top === 0 && el.offset().left === 0) {
        throw new Error(
            "el has a good probability to be in an invalid state. This seems to happen after a failed scrollTop on the parent scrollable, among other conditions"
        );
    }
 ...
 */
var computeScrollTarget = function (
    el,
    scrollableViewport,
    ...
    if (debugScrollUtils) {
        console.log(
            ...
            " scrollHeight: ",
            scrollableViewport[0].scrollHeight,
            ", height: ",
            scrollableViewport.height(),
            " scrollableViewportScrollTop: ",
            scrollableViewportScrollTop,
            " el info: ",
            "elViewPortOffset: ",
            elViewPortOffset,
...
var scrollToElement = function (el, callback, margin, animate, watch) {
    if (debugScrollUtils) {
        console.log(
            "scrollUtils::scrollToElement() called with: ",
            el,
            callback,
            margin,
            animate,
    ...
    if (!scrollableElement) {
        throw new Exception(
            "scrollToElement: Unable to find a scrollable element."
        );
    }
    if (!el) {
        console.warn(
            "scrollUtils::scrollToElement(): Warning: element to scroll to not found, aborting"
        );
        return;
    }
    if (!_.isFunction(scrollableElement.offset)) {
        console.warn(
            "scrollUtils::scrollToElement(): Warning: scrollableElement has no offset, aborting"
        );
        return;
    }
    if (scrollableElement.offset() === undefined) {
        console.warn(
            "scrollUtils::scrollToElement(): Warning: scrollableElement has no offset (hidden?), aborting"
        );
        if (debugScrollUtils) {
            console.log(
                "scrollUtils::scrollToElement(): el: ",
                el,
                "scrollableElement: ",
                scrollableElement
            );
    ...
    var processCallback = function () {
        try {
            var finalViewportOffset = getElementViewportOffset(
                elReference,
                scrollableElement
            );
            var differenceToTargetOffset =
                finalViewportOffset - desiredViewportOffset;
            if (debugScrollUtils) {
                console.log(
                    "scrollUtils::scrollToElement(): Final viewPort offset: ",
                    finalViewportOffset,
                    ", difference of: ",
                    differenceToTargetOffset,
                    " with desiredViewportOffset of ",
                    desiredViewportOffset
                );
            }
            if (animate && differenceToTargetOffset !== 0) {
                if (debugScrollUtils) {
                    console.log(
                        "scrollUtils::scrollToElement(): differenceToTargetOffset is not 0.  The content above the element may well have changed length during animation, retry scrolling with no animation"
                    );
                }
                ...
                if (debugScrollUtils) {
                    console.log(
                        "scrollUtils::scrollToElement(): POST_RETRY Final viewPort offset: ",
                        getElementViewportOffset(el, scrollableElement),
                        ", difference to target offset: ",
                        getElementViewportOffset(el, scrollableElement) -
                            desiredViewportOffset,
    ...
    if (animate) {
        scrollableElement.animate(
            ...
            { always: processCallback, duration: 800 }
        );
    } else {
        scrollableElement.scrollTop(scrollTarget);
        processCallback();
...
var _watchOffset = function (
    el,
    scrollableViewport,
    initialWatchProcessTimestampParam
) {
    var initialScrollableViewportHeight = scrollableViewport[0].scrollHeight;
    var initialElViewPortOffset = getElementViewportOffset(
        el,
        scrollableViewport
    );
    ...
    if (debugScrollUtils) {
        console.log(
            "scrollUtils::_watchOffset: initialized with initialScrollableViewportHeight:",
            initialScrollableViewportHeight,
            ", initialElViewPortOffset: ",
            initialElViewPortOffset
        );
    }
    ...
    var _watchScrollHandler = function (ev) {
        ...
        var newElViewPortOffset = getElementViewportOffset(
            el,
            scrollableViewport
        );
        if (newElViewPortOffset === initialElViewPortOffset) {
            if (debugScrollUtils) {
                console.log(
                    "scrollUtils::_watchScrollHandler: We are at initialElViewPortOffset, we probably just caught our own scrollEvent, ignoring."
                );
            }
            return;
        } else {
            if (debugScrollUtils) {
                console.log(
                    "scrollUtils::_watchScrollHandler: We are NOT at initialElViewPortOffset, we just caught a user or script scroll.  Suiciding. currentElViewPortOffset:",
                    newElViewPortOffset
                );
            }
            _watchSuicide();
    ...
    var _watchProcess = function () {
        var currentScrollableViewportHeight =
            scrollableViewport[0].scrollHeight;
        var currentElViewPortOffset = getElementViewportOffset(
            el,
            scrollableViewport
        );
        var scrollableViewportHeightChange =
            currentScrollableViewportHeight - initialScrollableViewportHeight;
        var elViewPortOffsetChange =
            currentElViewPortOffset - initialElViewPortOffset;
        ...
        var newWatchCallback = function () {
            ...
            if (newElViewPortOffset !== initialElViewPortOffset) {
                if (debugScrollUtils) {
                    console.warn(
                        "scrollUtils::_watchProcess: I was unable to scroll to initialOffset, this shouldn't normally happen.  Cowardly aborting watch."
                    );
                }
            } else {
                _watchOffset(
        ...
        if (debugScrollUtils) {
            console.log(
                "scrollUtils::_watchProcess: unixTimestamp - initialWatchProcessTimestamp: ",
                unixTimestamp - initialWatchProcessTimestamp
            );
        }

        if (
            unixTimestamp - initialWatchProcessTimestamp >
            maxWatchProcessDuration
        ) {
            if (debugScrollUtils) {
                console.log(
                    "scrollUtils::_watchProcess: maxWatchProcessDuration duration exceeded, stopping watch"
                );
            }
        } else if (elViewPortOffsetChange === 0) {
            if (debugScrollUtils) {
                console.log(
                    "scrollUtils::_watchProcess: Everything is fine, same position in viewport, continuing to watch"
                );
            }
            newWatchCallback();
            ...
            if (debugScrollUtils) {
                console.log(
                    "scrollUtils::_watchProcess: Position in viewport changed, and size of content in viewport also changed, readjusting"
                );
            }
            ...
            if (debugScrollUtils) {
                console.warn(
                    "scrollUtils::_watchProcess: Position in viewport changed, but size of viewport did NOT change (undetected user or JS initiated scrool?).  Cowardly aborting watch."
                );
            }
            //scrollToElement(el, newWatchCallback, initialElViewPortOffset);
        }
