...
class ReadSource extends View.extend({
    ...
    reimportSource(e) {
        e.preventDefault();
        e.stopPropagation();
        return Promise.resolve(this.model.doReimport())
            .then(function (resp) {
                if (_.has(resp, "error")) {
                    Growl.showBottomGrowl(
                        Growl.GrowlReason.ERROR,
                        i18n.gettext("There was a reimport error!")
                    );
                    console.error(
                        "Source " +
                            this.model.name +
                            " failed to reimport due to an internal server problem with response ",
                        resp
                    );
                }
                Growl.showBottomGrowl(
                    ...
                    i18n.gettext(
                        "Reimport has begun! It can take up to 15 minutes to complete."
                    )
                );
            })
            .catch(function (e) {
    ...
    reprocessSource(e) {
        e.preventDefault();
        e.stopPropagation();
        return Promise.resolve(this.model.doReprocess())
            .then(function (resp) {
                if (_.has(resp, "error")) {
                    Growl.showBottomGrowl(
                        Growl.GrowlReason.ERROR,
                        i18n.gettext("There was a reprocess error!")
                    );
                    console.error(
                        "Source " +
                            this.model.name +
                            " failed to reprocess due to an internal server problem with response",
                        resp
                    );
                }
                Growl.showBottomGrowl(
                    Growl.GrowlReason.SUCCESS,
                    i18n.gettext(
                        "Reprocess has begun! It can take up to 15 minutes to complete."
                    )
                );
            })
            .catch(function (e) {
...
class SourceView extends View.extend({
    ...
    onRender() {
        ...
        if (editViewClass !== undefined) {
            this.showChildView(
                "form",
                new editViewClass({ model: this.model })
            );
        }
    }
}
...
class CreateSource extends View.extend({
    ...
    serializeData() {
        ...
        var type_name_assoc = {};
        for (var i in types) {
            type_name_assoc[types[i]] = Source.getSourceClassByType(
                types[i]
