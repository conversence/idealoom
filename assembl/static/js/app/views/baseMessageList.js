...
return cls.extend({
  ...
  render_real: function() {
    ...
    var partialMessage = MessagesInProgress.getMessage(partialMessageContext);
  ...
  _generateVisitorData: function(messageStructureCollection, resultMessageIdCollection) {
    ...
    var visitorObject = new MessageRenderVisitor(visitorViewData, tempVisitorOrderLookupTable, tempVisitorRootMessagesToDisplay, inFilter);
  ...
  initAnnotator: function() {
    ...
    this.annotator.subscribe('annotationEditorShown', function(annotatorEditor, annotation) {
      ...
      var div = $('<div/>');
      var div_draggable = $('<div/>', { 'draggable': true, 'class': 'annotator-textarea' });
      var div_annotator_help = i18n.sprintf("<div class='annotator-draganddrop-help'>%s</div>", i18n.gettext('You can drag the segment below directly to the table of ideas'));
      var div_copy_paste = i18n.sprintf("<div class='annotator-draganddrop-help'>%s</div><div class='annotator-copy-paste-zone'>%s</div>", i18n.gettext('You can also copy-paste from the text in the zone below'), annotation.quote);
  ...
  showMessageById: function(id, callback, shouldHighlightMessageSelected, shouldOpenMessageSelected, shouldRecurseMaxMoreTimes, originalRenderId) {
    ...
    Promise.join(this.currentQuery.getResultMessageStructureCollectionPromise(),
        ...
        function(allMessageStructureCollection, visitorData, resultMessageIdCollection) {
          ...
          var messageIsInFilter = that.isMessageIdInResults(id, resultMessageIdCollection);
          var requestedOffsets;
  ...
  getMessageGeometry: function (messageSelector, overrideMessageTop) {
    ...
    let messageTop = overrideMessageTop ? overrideMessageTop : messageSelector.offset().top;
  ...
  getMessageVsViewportGeometry: function (messageGeometry, viewportGeometry) {
    ...
    const { currentViewPortTop,
      currentViewPortBottom,
      viewportHeight } = viewportGeometry;
    ...
    msgFractionInsideViewPort = 1 - msgFractionAboveViewPort - msgFractionBelowViewPort;
    const msgVsViewportRatio = messageHeight / viewportHeight;
    const viewportFractionCoveredByMsg = (msgFractionInsideViewPort * messageHeight) / viewportHeight;
  ...
  checkMessagesOnscreen: function (resultMessageIdCollection, visitorData) {
    ...
    let currentScrollTop = this.ui.panelBody.scrollTop();
    if (!currentScrollTop) {
    ...
    const { currentViewPortTop,
      currentViewPortBottom } = viewportGeometry;

    if (false && this.debugScrollLogging) {
      //console.log(messageDoms);
      console.log("checkMessagesOnscreen() starts at currentScrollTop:", currentScrollTop);
    }

    let latestVectorAtStart = that.scrollLoggerVectorStack[that.scrollLoggerVectorStack.length - 1];
    let previousVectorAtStart = that.scrollLoggerVectorStack[that.scrollLoggerVectorStack.length - 2];
    _.each(messageDoms, function (messageSelector) {
      ...
      const messageVsViewportGeometry = that.getMessageVsViewportGeometry(messageGeometry, viewportGeometry);
      ...
      if (updateType !== ATTENTION_UPDATE_NONE) {
        ...
        switch (updateType) {
          ...
          case ATTENTION_UPDATE_INTERIM:
            timeStampFirstOnscreen = existingLog.timeStampFirstOnscreen;
            timeStampLastOnscreen = latestVectorAtStart.timeStamp;
            messageTopWhenFirstOnscreen = existingLog._internal.messageTopWhenFirstOnscreen;
            break;
        ...
        const vectors = that.getScrollVectorsForTimeRange(
          timeStampFirstOnscreen, timeStampLastOnscreen);
        ...
        const vectorsInfo = that.processScrollVectors(that.scrollLoggerMessages[messageId], vectors, messageSelector, viewportGeometry);

        let retVal = { _internal: _, ...that.scrollLoggerMessages[messageId], ...vectorsInfo.metrics }

        if (that.debugScrollLogging) {
          console.log("Mock-wrote new log for message", messageId, ": ", retVal);
        }
      }
  ...
  processScrollVectors: function (scrollLoggerMessageInfo, vectors, messageSelector, viewportGeometry) {
    ...
    const reducer = (acc, vector, idx, vectors) => {
      ...
      if (idx !== 0) {//Skip the first vector, as it's the one BEFORE the message was onscreen
        ...
        */
        const netDistanceScrolledSoFar = acc.metrics.cumulativeUpDistancePx - acc._internal.cumulativeDownDistancePx;
        const messageTopAtStartOfVector = messageTopWhenFirstOnscreen - netDistanceScrolledSoFar;
        const messageGeometry = that.getMessageGeometry(messageSelector, messageTopAtStartOfVector);

        const messageVsViewportGeometry = that.getMessageVsViewportGeometry(messageGeometry, viewportGeometry);
        const { msgFractionInsideViewPort: msgFractionInsideViewPortAtStartOfVector,
          msgScrollableDistance } = messageVsViewportGeometry;
        ...
        if (direction !== DIRECTION_NONE) { //We actually moved
        ...
        if (direction !== DIRECTION_NONE && //We actually moved
          ...
          acc.metrics.numEstimatedInternalDirectionChanges += 1 * viewportFractionCoveredByMsg;
        }

        let duration = vector.timeStamp - previousVector.timeStamp;
        acc.metrics.totalDurationMs += duration;
        if (direction === DIRECTION_DOWN) {
          acc._internal.cumulativeDownDistancePx += Math.abs(distancePx);
        }
        if (direction === DIRECTION_UP) {
          acc._internal.cumulativeUpDistancePx += Math.abs(distancePx);
          ...
          1400px: 1400/1100
          */
          const currEffectiveFractionOfMessageScrolled = distancePx / msgScrollableDistance;
          acc.metrics.totalEffectiveFractionOfMessageScrolled += currEffectiveFractionOfMessageScrolled;
          const fractionOfTotalUpScroll = distancePx / acc._internal.cumulativeUpDistancePx;
          ...
          if (acc.metrics.scrollUpAvgWPM) {
            acc.metrics.scrollUpAvgWPM = fractionOfTotalUpScroll * scrollWordsPerMinute + (1 - fractionOfTotalUpScroll) * acc.metrics.scrollUpAvgWPM;
          } else {
            acc.metrics.scrollUpAvgWPM = scrollWordsPerMinute;
          }
          //Règle de 3...
          acc.metrics.effectiveWpmIfUserReadWholeMessage = acc.metrics.scrollUpAvgWPM
            / acc.metrics.totalEffectiveFractionOfMessageScrolled;
        }

        if (false && that.debugScrollLogging) {
          console.log("reducer(): DEBUG for idx", idx, ":", { ...acc.metrics });
        }
      }
      return acc;
    };
