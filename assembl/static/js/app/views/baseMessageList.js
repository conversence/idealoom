...
function BaseMessageListMixinFactory(cls) {
   ...
   */
     ...
     */
        ...
        getScrollVectorsForTimeRange(
            ...
            let applicableVectors = that.scrollLoggerVectorStack.filter(
                checkApplicability
            );
        ...
        getMessageGeometry(messageSelector, overrideMessageTop) {
            if (!messageSelector || messageSelector.length == 0) {
                throw new Error(
                    "getMessageGeometry(): messageSelector is invalid"
                );
            }
      ...
      */
        ...
        getMessageVsViewportGeometry(messageGeometry, viewportGeometry) {
            ...
            let bottomDistanceBelowViewPort =
                messageBottom - currentViewPortBottom;
  ...
  "\nmsgFractionInsideViewPort", msgFractionInsideViewPort );*/
        ...
        checkMessagesOnscreen(resultMessageIdCollection, visitorData) {
            let that = this;
            let messageDoms = this.getOnScreenMessagesSelectors(
                resultMessageIdCollection,
                visitorData
            );
            ...
            let latestVectorAtStart =
                that.scrollLoggerVectorStack[
                    that.scrollLoggerVectorStack.length - 1
                ];
            let previousVectorAtStart =
                that.scrollLoggerVectorStack[
                    that.scrollLoggerVectorStack.length - 2
                ];
                    ...
                    switch (updateType) {
                        ...
                        case ATTENTION_UPDATE_FINAL: //Basically, repeat last interim
                            timeStampFirstOnscreen =
                                existingLog.timeStampFirstOnscreen;
                            timeStampLastOnscreen =
                                previousVectorAtStart.timeStamp;
                            messageTopWhenFirstOnscreen =
                                existingLog._internal
                                    .messageTopWhenFirstOnscreen;
                            break;
        ...
        pruneOldScrollVectors(timeStamp) {
            ...
            let firstNewerVectorIndex = that.scrollLoggerVectorStack.findIndex(
                (vector) => {
     ...
     */
            ...
            const reducer = (acc, vector, idx, vectors) => {
                if (idx !== 0) {
                    ...
                    let distancePx =
                        vector.scrollTop - previousVector.scrollTop;
    ...
    BaseMessageListMixin.scrollLogger = _.debounce(
        ...
        SCROLL_VECTOR_MEASUREMENT_INTERVAL,
        false
    );
