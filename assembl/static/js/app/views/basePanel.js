...
class BasePanel extends LoaderView.extend({
    ...
    getContainingGroup() {
        if (this._panelWrapper.groupContent) {
            //Not using instanceof GroupContent here to avoid requirejs circular dependency.
            return this._panelWrapper.groupContent;
        } else {
            throw new Error(
                "The panel wasn't initialized properly (most likely initialize wasn't called by the extended class"
            );
        }
    }
