...
class groupContent extends View.extend({
    ...
    NavigationResetVisualizationState(url) {
        if (!this.isDestroyed()) {
            ...
            if (nav) {
                ...
                this.ensureOnlyPanelsVisible(
                    PanelSpecTypes.EXTERNAL_VISUALIZATION_CONTEXT
                );
                var vizPanel = this.findViewByType(
                    PanelSpecTypes.EXTERNAL_VISUALIZATION_CONTEXT
                );
                vizPanel.setUrl(url);
            }
    ...
    ensureOnlyPanelsVisible() {
        ...
        _.each(this.model.get("panels").models, function (aPanelSpec) {
            ...
            if (panelSpecType === PanelSpecTypes.NAV_SIDEBAR) return;
    ...
    ensurePanelsVisible() {
        ...
        this.model.ensurePanelsAt(args, 1);
        // show and hide panels
        var panelSpecsToMakeVisible = this.model
            .get("panels")
            .models.filter(function (aPanelSpec) {
                var panelSpecType = aPanelSpec.getPanelSpecType();
    ...
    getGroupStoragePrefix() {
        ...
        var groupContentIndexInGroupContainer = groupContent.groupContainer.collection.indexOf(
            groupContent.model
        );
        var storagePrefix =
            Storage.getStoragePrefix() +
            "_group_" +
            groupContentIndexInGroupContainer;
    ...
    applyUserCustomDataChangesOnGroupClose() {
        ...
        if (groupContentIndexInGroupContainer < sz - 1) {
            ...
            for (i = groupContentIndexInGroupContainer; i < sz; ++i) {
                ...
                tableOfIdeasCollapsedStatePromises[
                    i
                ] = tableOfIdeasCollapsedStateModels[i].fetch();
            }
            Promise.all(tableOfIdeasCollapsedStatePromises).then(function (
                models
            ) {
                for (i = groupContentIndexInGroupContainer; i < sz; ++i) {
                    console.log(
                        "tableOfIdeasCollapsedStateModels[i]: ",
                        tableOfIdeasCollapsedStateModels[i]
                    );
                    // Move table of ideas collapsed state of group number i to one of group number i-1
                    tableOfIdeasCollapsedStateModels[i].set(
                        "id",
                        storagePrefix + (i - 1) + storageSuffix
                    );
