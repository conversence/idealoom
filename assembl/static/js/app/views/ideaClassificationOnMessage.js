    ...
    initialize(options) {
        ...
        Promise.join(
            ...
            function (postCreator, linkCreator, idea, langPrefs) {
                ...
                if (idExcerpt) {
                    return idea.collection.collectionManager
    ...
    onRender() {
        if (this.canRender) {
            if (!this.ideaAncestry) {
                throw new Error(
                    "Idea Ancestry Collection was undefined on message ",
                    this.model.get("idPost")
                );
            }
   ...
   */
    ...
    onSeeIdeaClick() {
        ...
        analytics.trackEvent(
            analytics.events.NAVIGATE_TO_IDEA_IN_IDEA_CLASSIFICATION
        );
...
class ErrorView extends View.extend({
    ...
    initialize(options) {
        console.error(
            "[IdeaClassificationModal] An error view was created on the idea content link",
            this.model.id
        );
    }
...
class IdeaShowingMessageCollectionView extends View.extend({
    ...
    onRender() {
        this.showChildView(
            "items",
            new IdeaShowingMessageCollectionViewBody({
...
class IdeasShowingMessageModal extends Backbone.Modal.extend({
    ...
    serializeData() {
        ...
        return {
            ...
            title_msg: i18n.ngettext(
                ...
                "Links between this message and ideas",
                number_of_ideas
            ),
        };
    }
    ...
    onRender() {
        ...
        this.$(".ideas-reasons-collection").html(
            IdeaClassificationCollectionView.render().el
        );
    }
}
