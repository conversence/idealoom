...
class IdeaClassificationNameListView extends LoaderView.extend({
    ...
    initialize(options) {
        ...
        var that = this;
        var ideaContentLinks = cm.getIdeaContentLinkCollectionOnMessage(
            this.model
        );
    ...
    serializeData() {
        ...
        return {
            ...
            linked_to_msg: i18n.ngettext(
                ...
                "This message is linked to the following ideas: ",
                count
            ),
    ...
    onIdeaClick(e) {
        ...
        analytics.trackEvent(
            analytics.events.NAVIGATE_TO_CLASSIFICATION_ON_MESSAGE
        );
...
class MessageView extends LoaderView.extend({
    ...
    ui: {
        ...
        jumpToMessageInReverseChronologicalButton:
            ".js_message-jump-to-message-in-reverse-chronological",
        showAllMessagesByThisAuthorButton:
            ".js_message-show-all-by-this-author",
    ...
    events: {
        ...
        "click @ui.jumpToMessageInThreadButton":
            "onMessageJumpToMessageInThreadClick",
        "click @ui.jumpToMessageInReverseChronologicalButton":
            "onMessageJumpToMessageInReverseChronologicalClick",
        "click @ui.showAllMessagesByThisAuthorButton":
            "onShowAllMessagesByThisAuthorClick",
        ...
        //
        "mousedown  @ui.messageBodyAnnotatorAllowed":
            "startAnnotatorTextSelection",
        "mousemove  @ui.messageBodyAnnotatorAllowed":
            "updateAnnotatorTextSelection",
        "mouseleave @ui.messageBodyAnnotatorAllowed":
            "onMouseLeaveMessageBodyAnnotatorSelectionAllowed",
        "mouseenter @ui.messageBodyAnnotatorAllowed":
            "updateAnnotatorTextSelection",
     ...
     console.log("message model event received: ", eventName);
        ...
        if (!this.isDestroyed()) {
            ...
            this.listenTo(
                ...
                "annotator:destroy",
                this.onAnnotatorDestroy
            );
            this.listenTo(
                this.messageListView,
                "annotator:initComplete",
                this.onAnnotatorInitComplete
            );
            this.listenTo(
                this.messageListView,
                "annotator:success",
    ...
    isCompleteDataLoaded() {
        ...
        if (!this.isLoading() && this.model.get("@view") === "default") {
            ...
            Promise.join(
                ...
                this.model.collection.collectionManager.getMessageFullModelPromise(
                    this.model.id,
                    Promise.resolve(this.model.collection)
                ),
    ...
    guardedRender() {
        if (Ctx.debugRender) {
            console.log(
                "MessageView modelEvents change fired from",
                this.model
            );
        }
        if (!this.isDestroyed()) {
    ...
    processContent() {
        ...
        if (this.hasTranslatorService) {
            if (!this.useOriginalContent) {
                var processedBody = body.bestWithErrors(
                    this.translationData,
                    false
                );
                var processedSubject = subject.bestWithErrors(
                    this.translationData,
                    true
                );
                ...
                if (processedBody.error == 13) {
                    ...
                    this.unknownPreference =
                        this.translationData
                            .getPreferenceForLocale(
                                processedBody.entry.getOriginalLocale()
                            )
                            .get("source_of_evidence") === null;
    ...
    serializeData() {
        ...
        if (
            ...
            this.viewStyle === this.availableMessageViewStyles.TITLE_ONLY
        ) {
        ...
        var isModerated = publication_state
            ? publication_state in MessageModel.ModeratedPublicationStates
            : this.moderationOptions.isModerated;
        if (isModerated) {
        ...
        if (this.model.getBEType() == Types.SYNTHESIS_POST) {
            html_export_url = Ctx.getApiV2DiscussionUrl(
                "posts/" + this.model.getNumericId() + "/html_export"
            );
        }

        var user_can_delete_this_message = Ctx.getCurrentUser().can(
            Permissions.DELETE_POST
        ); // TODO: check local permissions on post
        ...
        return {
            ...
            user_can_export_post: Ctx.getCurrentUser().can(
                Permissions.EXPORT_EXTERNAL_SOURCE
            ),
            ...
            user_can_moderate: Ctx.getCurrentUser().can(
                Permissions.MODERATE_POST
            ),
    ...
    renderLikeCount() {
        ...
        if (Ctx.debugRender) {
            console.log(
                "message:renderLikeCount() is firing for message",
                this.model.id
            );
        }
        if (
            this.messageListView.currentViewStyle ==
            this.messageListView.ViewStyles.POPULARITY
        ) {
    ...
    onBeforeRender() {
        ...
        if (
            ...
            this.moderationOptions.purpose = this.model.get(
                "publication_state"
            );
        }
    }
    ...
    onRender() {
        ...
        if (
            publication_state &&
            publication_state in MessageModel.DeletedPublicationStates
        ) {
            ...
            if (Ctx.debugLangstring) {
                ...
                console.log(
                    ...
                    this.bodyTranslationError
                );
                console.log("isMessageTranslated: ", this.isMessageTranslated);
                console.log(
                    "Discrepency? ",
                    !(this.isMessageTranslated !== this.showAnnotations)
                );
                console.log("_body.value: ", this._body.value());
                console.log(
                    "_body.isMachineTranslation(): ",
                    this._body.isMachineTranslation()
                );
            }
            ...
            if (Ctx.debugRender) {
                console.log(
                    "message:render() is firing for message",
                    this.model.id
                );
            }
            ...
            if (this.viewStyle === this.availableMessageViewStyles.FULL_BODY) {
                ...
                Ctx.makeLinksShowOembedOnHover(
                    this.$el.children(".message-body")
                );
            }
            if (this.model.get("metadata_json")) {
                Ctx.makeLinksShowOembedOnHover(
                    this.$el.find(".inspirationSource")
                );
            }
            ...
            if (
                this.viewStyle === that.availableMessageViewStyles.FULL_BODY &&
                (this.replyBoxShown || partialMessage.body)
            ) {
            ...
            if (
                this.viewStyle === this.availableMessageViewStyles.FULL_BODY ||
                this.viewStyle === this.availableMessageViewStyles.PREVIEW
            ) {
                if (this.canShowTranslation()) {
                    if (
                        (this.forceTranslationQuestion &&
                            !this.hideTranslationQuestion) ||
                        ...
                        var translationView = new MessageTranslationView({
                            messageModel: this.model,
                            messageView: this,
                        });
                        this.showChildView(
                        ...
                        this.getRegion("translationRegion").$el.removeClass(
                            "hidden"
                        );
                    } else if (this.getRegion("translationRegion").$el) {
                        this.getRegion("translationRegion").$el.addClass(
                            "hidden"
                        );
                    }
                }
            ...
            if (
                this.viewStyle === that.availableMessageViewStyles.FULL_BODY &&
                this.messageListView.defaultMessageStyle !==
                    this.availableMessageViewStyles.FULL_BODY
            ) {
                this.showReadLess();
            }

            if (
                this.messageListView.isCurrentViewStyleThreadedType() &&
                that.messageFamilyView.currentLevel !== 1
            ) {
                this.model
                    .getParentPromise()
                    .then(function (parentMessageModel) {
                        //console.log("comparing:", parentMessageModel.getSubjectNoRe(), that.model.getSubjectNoRe());
                        if (
                            parentMessageModel &&
                            parentMessageModel.getSubjectNoRe() ===
                                that.model.getSubjectNoRe()
                        ) {
            ...
            if (this.viewStyle === this.availableMessageViewStyles.PREVIEW) {
                var applyEllipsis = function () {
                    ...
                    that.$(".ellipsis").dotdotdot({
                        ...
                        callback: function (isTruncated, orgContent) {
                            ...
                            if (isTruncated) {
                                that.$(
                                    ".ellipsis > a.readMore, .ellipsis > p > a.readMore"
                                ).removeClass("hidden");
                            } else {
                                that.$(
                                    ".ellipsis > a.readMore, .ellipsis > p > a.readMore"
                                ...
                                if (
                                    that.model.get("body") &&
                                    that.model.get("body").length > 610
                                ) {
                                    // approximate string length for text which uses 4 full lines
                                    if (Ctx.debugRender) {
                                        console.log(
                                            "there may be a problem with the dotdotdot of message ",
                                            that.model.id,
                                            "so we will maybe re-render it"
                                        );
                                    }

                                    if (++that.reRendered < 5) {
                                        // we use this to avoid infinite loop of render() calls
                                        if (Ctx.debugRender) {
                                            console.log(
                                                "yes, we will re-render => tries: ",
                                                that.reRendered
                                            );
                                        }
                                        ...
                                        if (Ctx.debugRender) {
                                            console.log(
                                                "no, we won't re-render it because we already tried several times: ",
                                                that.reRendered
                                            );
                                        }
                                    }
                ...
                that.messageListView.requestPostRenderSlowCallback(function () {
                    setTimeout(function () {
                        ...
                        var current_navigation_state = that.messageListView
                            .getContainingGroup()
                    ...
                    that.listenTo(
                        ...
                        function (navSection) {
                            ...
                            if (navSection === "debate") {
                                ...
                                that.messageListView.requestPostRenderSlowCallback(
                                    function () {
                                        that.$(".ellipsis").trigger(
                                            "update.dot"
                                        );
                                    }
                                );
            ...
            Promise.resolve(
                $.ajax(
                    Ctx.getApiV2DiscussionUrl(
                        "posts/" + this.model.getNumericId() + "/actions"
                    ),
                    {
                        method: "POST",
        ...
        return annotationsPromise.then(function (annotations) {
            if (that.annotationsToLoad === undefined) {
                ...
                if (!that.canShowAnnotations()) {
                    annotations = annotations.filter((extract) =>
                        extract.getNumLinkedIdeas()
                    );
                }
    ...
    loadAnnotations() {
        ...
        if (
            ...
            this.getAnnotationsToLoadPromise().done(function (
                annotationsToLoad
            ) {
                if (!that.isDestroyed()) {
                    ...
                    if (annotationsToLoad.length) {
                        ...
                        that.annotator.loadAnnotations(
                            _.clone(annotationsToLoad)
                        );
                        _.each(annotationsToLoad, function (annotation) {
                            that.loadedAnnotations[
                                annotation["@id"]
                            ] = annotation;
                        });
                        setTimeout(function () {
        ...
        if (this.showAnnotations) {
            ...
            if (this.annotator && this.loadedAnnotations) {
                for (var annotation_id in this.loadedAnnotations) {
                    this.annotator.deleteAnnotation(
                        this.loadedAnnotations[annotation_id]
                    );
                }
            }
        }
   ...
   */
    ...
    showSegmentByAnnotation(annotation) {
        const that = this;
        const currentIdea = this.messageListView
            .getGroupState()
            .get("currentIdea");
        const collectionManager = new CollectionManager();
        collectionManager
            .getAllExtractsCollectionPromise()
            .then((allExtractsCollection) => {
                const segment = allExtractsCollection.getByAnnotation(
                    annotation
                );
                if (!segment) {
                    console.error(
                        "message::showSegmentByAnnotation(): the extract doesn't exist"
                    );
                    return;
                }
                const currentLink = currentIdea
                    ? segment.linkedToIdea(currentIdea.getId())
                    : null;

                if (currentLink) return;

                if (segment.getNumLinkedIdeas() == 0) {
                    if (
                        that.messageListView
                            .getContainingGroup()
                            .findViewByType(PanelSpecTypes.CLIPBOARD)
                    ) {
                        //FIXME:  We don't want to affect every panel, only the one in the current group
                        //FIXME:  Nothing listens to this anymore
                        console.error(
                            "FIXME:  Nothing listens to DEPRECATEDsegmentList:showSegment anymore"
                        );
                        IdeaLoom.other_vent.trigger(
                            "DEPRECATEDsegmentList:showSegment",
                            segment
                        );
                    } else {
                        console.log(
                            "TODO:  NOT implemented yet.  Should pop panel in a lightbox.  See example at the end of Modal object in navigation.js "
                        );
                    }
                    return;
                }
                ...
                class Modal extends Backbone.Modal.extend({
                    template: _.template(
                        $("#tmpl-showSegmentByAnnotation").html()
                    ),
    ...
    renderAttachments() {
        ...
        if (this.canShowAttachments()) {
            this.showChildView(
                "attachmentsRegion",
                this.attachmentsCollectionView
            );
        }
    }
    ...
    showAnnotatorSelectionTooltip(x, y, text) {
        ...
        text =
            text.substr(0, TOOLTIP_TEXT_LENGTH) +
            "..." +
            text.substr(-TOOLTIP_TEXT_LENGTH);
    ...
    focusReplyBox() {
        ...
        if (!this.isMessageOpened()) {
            console.error(
                "Tried to focus on the reply box of a closed message, this should not happen!"
            );
        }

        if (!this.replyBoxHasFocus) {
            console.error(
                "Tried to focus on the reply box of a message that isn't supposed to have focus, this should not happen!"
            );
        }
        if (this.replyView) {
            //FIX ME
            ...
            if (el instanceof jQuery && el.length) {
                ...
                setTimeout(function () {
                    if (!that.isDestroyed()) {
                        if (Ctx.debugRender) {
                            console.log(
                                "Message:focusReplyBox() stealing browser focus"
                            );
                        }
            ...
            console.log("Global focusin", event);
                        ...
                        var maintainFocus = function (ev) {
                            if (!that.isDestroyed() && !el.is(":focus")) {
                                if (Ctx.debugRender) {
                                    console.warn(
                                        "focus was quickly lost.  Focusing again to work around the problem."
                                    );
                                }
                                setTimeout(function () {
                                    el.focus();
        ...
        if (this.ui.messageReplyBox.length) {
            ...
            console.error(
                "Tried to focus on the reply box of a message, but reply box isn't onscreen.  This should not happen!"
            );
        }
    }
    ...
    onMessageHoistClick(ev) {
        ...
        this.isHoisted = this.messageListView.toggleFilterByPostId(
            this.model.getId()
        );
    ...
    onMessageJumpToMessageInThreadClick(ev) {
        ...
        this.messageListView.setViewStyle(
            this.messageListView.ViewStyles.NEW_MESSAGES
        );
    ...
    onModerationOptionsClick(ev) {
        ...
        this.showChildView(
            "moderationOptionsRegion",
            this.messageModerationOptionsView
        );
        this.listenToOnce(
            this.messageModerationOptionsView,
            "moderationOptionsSaveAndClose",
            this.onModerationOptionsSaveAndClose
        );
        this.listenToOnce(
            this.messageModerationOptionsView,
            "moderationOptionsClose",
            this.onModerationOptionsClose
        );
    }
            ...
            Promise.resolve(
                ...
                .then(function (data) {
                    ...
                    that.fetchIdeasCollection().then(function () {
                        ...
                        that.messageListView.showMessageById(
                            that.model.id,
                            null,
                            true,
                            false
    ...
    onMessageJumpToMessageInReverseChronologicalClick(ev) {
        this.messageListView.currentQuery.clearAllFilters();
        this.messageListView.setViewStyle(
            this.messageListView.ViewStyles.REVERSE_CHRONOLOGICAL
        );
        this.messageListView.render();
        this.messageListView.showMessageById(this.model.id);
    }
    ...
    onShowAllMessagesByThisAuthorClick(ev) {
        this.messageListView.currentQuery.clearAllFilters();
        this.messageListView.currentQuery.addFilter(
            this.messageListView.currentQuery.availableFilters.POST_IS_FROM,
            this.model.get("idCreator")
        );
        this.messageListView.render();
        this.messageListView.showMessageById(this.model.id);
    }
    ...
    doCloseMessage() {
        if (this.isMessageOpened()) {
            this.setViewStyle(
                this.messageListView.getTargetMessageViewStyleFromMessageListConfig(
                    this
                )
            );
            this.replyBoxShown = false;
    ...
    onMessageTitleClick(e) {
        if (e) {
            ...
            if (
                target.is("a") &&
                !(
                    target.hasClass("js_readMore") ||
                    target.hasClass("js_readLess")
                )
            ) {
                return;
            }
    ...
    startAnnotatorTextSelection() {
        ...
        if (
            this.messageListView.isInPrintableView() ||
    ...
    updateAnnotatorTextSelection(ev) {
        ...
        var text = selectedText.focusNode
            ? selectedText.getRangeAt(0).cloneContents()
            : "";
    ...
    onMouseLeaveMessageBodyAnnotatorSelectionAllowed() {
        if (Ctx.debugAnnotator) {
            console.log(
                "onMouseLeaveMessageBodyAnnotatorSelectionAllowed called"
            );
        }
    ...
    finishAnnotatorTextSelection(ev) {
        ...
        if (
            this.isSelecting &&
            text.length > MIN_TEXT_TO_TOOLTIP &&
            isInsideAMessage
        ) {
            if (user.can(Permissions.ADD_EXTRACT)) {
                this.showAnnotatorSelectionSaveOptions(
                    ev.clientX - 50,
                    ev.clientY
                );
            } else {
                console.warn(
                    "finishAnnotatorTextSelection() called but current user does not have Permissions.ADD_EXTRACT"
                );
            }
        }
    ...
    closeTranslationView(cb) {
        ...
        Genie.geniefy($source, $target, 500).then(function () {
   ...
   */
    ...
    canShowAttachments() {
        if (this.moderationOptions.isModerated) {
            if (
                this.moderationOptions.purpose ===
