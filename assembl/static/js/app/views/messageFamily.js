...
class MessageFamilyView extends LoaderView.extend({
    ...
    events: {
        ...
        "click >.message-conversation-block>.js_viewMessageFamilyConversation":
    ...
    serializeData() {
        ...
        if (this.messageListView.isCurrentViewStyleThreadedType()) {
            if (
                visitorViewData.filtered_descendant_count !==
                    visitorViewData.real_descendant_count ||
                (visitorViewData.real_ancestor_count !==
                    visitorViewData.level &&
                    firstMessage.get("parentId") &&
                    this.level === 1)
            ) {
                hasParentsOrChildrenOutOfScope = true;
                numAncestorsOutOfContext =
                    visitorViewData.real_ancestor_count - visitorViewData.level;
                numDescendantsOutOfContext =
                    visitorViewData.real_descendant_count -
                    visitorViewData.filtered_descendant_count;
                numAuthorsOutOfContext =
                    visitorViewData.real_descendant_authors_list.length -
                    visitorViewData.filtered_descendant_authors_list.length +
                    visitorViewData.real_ancestor_authors_list.length -
                    visitorViewData.filtered_ancestor_authors_list.length;
            }
        } else {
            if (
                visitorViewData.real_descendant_count > 0 ||
                visitorViewData.real_ancestor_count > 0
            ) {
                hasParentsOrChildrenOutOfScope = true;
                ...
                numAuthorsOutOfContext =
                    _.union(
                        visitorViewData.real_descendant_authors_list,
                        visitorViewData.real_ancestor_authors_list,
        ...
        return {
            ...
            ctxMessageCountTooltip: i18n.sprintf(
                i18n.ngettext(
            ...
            ctxAuthorCountTooltip: i18n.sprintf(
                i18n.ngettext(
    ...
    onRender() {
        ...
        if (
            ...
            if (
                publication_state ===
                MessageModel.PublicationStates.DELETED_BY_USER
            ) {
                messageViewClass = MessageDeletedByUserView;
            } else {
                // else if ( publication_state == MessageModel.PublicationState.DELETED_BY_ADMIN ){
        ...
        this.messageListView.renderedMessageViewsCurrent[
            this.model.id
        ] = this._messageView;
        ...
        this.$el
            .find(">.message-family-arrow>.message")
            .replaceWith(this._messageView.el);
    ...
    onViewConversationClick(ev) {
        ...
        analytics.trackEvent(
            analytics.events.THREAD_VIEW_COMPLETE_CONVERSATION
        );

        var filters = [
            {
                filterDef:
                    availableFilters.POST_IS_DESCENDENT_OR_ANCESTOR_OF_POST,
                value: this.model.id,
            },
        ];
        ...
        var modal_title = i18n.sprintf(
            i18n.gettext('Zooming on the conversation around "%s"'),
