...
var PostQuery = function () {
    ...
    this.getFilterDefById = function (filterDefId) {
        ...
        throw new Error(
            "getFilterDefById(): No filter definition with id " + filterDefId
        );
    };
    ...
    this.addFilter = function (filterDef, value) {
        ...
        if (this.isFilterInQuery(filterDef, value)) {
            ...
            if (_.isArray(incompatibleFilters) && incompatibleFilters.length) {
                _.each(incompatibleFilters, function (id) {
    ...
    this.setView = function (view) {
        ...
        if (view) {
            if (
                this._view._server_order_param_value !==
                view._server_order_param_value
            ) {
                this._view = view;
                this.invalidateResults();
            }
    ...
    this._execute = function () {
        ...
        if (this._resultsAreValid) {
            ...
            return Promise.resolve($.getJSON(url, params)).then(function (
                data
            ) {
    ...
    this.getResultMessageStructureCollectionPromise = function () {
        ...
        if (
            "only_deleted_posts" in this._query ||
            "also_deleted_posts" in this._query
        ) {
            var url = Ctx.getApiUrl("posts");
    ...
    this._getHtmlFilterDescriptionPromise = function (filter) {
        ...
        _.each(filter.getValues(), function (value) {
            individualValuesButtonsPromises.push(
                filter
                    .getFilterIndividualValueDescriptionStringPromise(value)
                    .then(function (individualValueString) {
                        var retval = "";
                        retval += "<span>" + individualValueString + "</span>";
                        retval +=
                            '<a href="#" class="remove js_deleteFilter" data-filterid="' +
                            filter.getId() +
                            '" data-value-index="' +
                            _.indexOf(filter.getValues(), value) +
        ...
        descriptionPromise = filter.getFilterDescriptionStringPromise(
    ...
    this._getHtmlFiltersDescriptionPromise = function () {
        ...
        _.each(this._query, function (filter) {
            filterDescriptionPromises.push(
                that._getHtmlFilterDescriptionPromise(filter)
            );
        });

        return Promise.all(filterDescriptionPromises).then(function (
            filterDescriptions
        ) {
            var retval = "";
            ...
            if (nActiveFilters > 0) {
                retval +=
                    '<div class="actions"><a class="js_messageList-allmessages btn btn-cancel btn-xs">' +
                    i18n.gettext("Clear filters") +
                    "</a></div>";
            }
    ...
    this.getHtmlDescriptionPromise = function () {
        var that = this;
        return Promise.join(
            this._getHtmlFiltersDescriptionPromise(),
            this.getTotalNumMessages(),
            function (filtersDescription, totalMsgCount) {
                var retval = "";
                ...
                if (that._queryResultInfo == null) {
                    ...
                    if (that.getResultNumTotal() == 0) {
                        if (numActiveFilters > 0) {
                            ...
                            retval += i18n.gettext(
                                "There are no messages in the discussion."
                            );
                        }
                    } else {
                        var unreadText = "";
                        ...
                        if (numActiveFilters > 0) {
                            retval += i18n.sprintf(
                                i18n.ngettext(
                                    "Found %d message%s out of %d messages that:",
                                    "Found %d messages%s out of %d messages that:",
                                    that.getResultNumTotal(),
                                    totalMsgCount
                                ),
                                that.getResultNumTotal(),
                            ...
                            retval += i18n.sprintf(
                                i18n.ngettext(
                                    "Found %d message%s out of %d messages:",
                                    "Found %d messages%s out of %d messages:",
                                    that.getResultNumTotal(),
                                    totalMsgCount
                                ),
                                that.getResultNumTotal(),
                    ...
                    throw new Error(
                        "Query has been executed but results are invalid"
                    );
                }
