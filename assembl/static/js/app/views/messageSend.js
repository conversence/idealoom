...
class messageSendView extends LoaderView.extend({
    ...
    initialize(options) {
        ...
        this.initialBody =
            this.options.body_help_message !== undefined
                ? this.options.body_help_message
                : i18n.gettext("Type your message here...");
        ...
        this.attachmentsCollection = new Attachments.Collection([], {
            objectAttachedToModel: this.model,
        });
    ...
    serializeData() {
        ...
        var show_cancel_button =
            "show_cancel_button" in this.options
                ? this.options.show_cancel_button
                : false;
        var reply_idea =
            "reply_idea" in this.options ? this.options.reply_idea : null;
        var reply_message_id =
            "reply_message_id" in this.options
                ? this.options.reply_message_id
                : null;
        var show_target_context_with_choice =
            "show_target_context_with_choice" in this.options
                ? this.options.show_target_context_with_choice
                : null;
        ...
        return {
            ...
            allow_setting_subject:
                this.options.allow_setting_subject ||
                this.options.allow_setting_subject,
            cancel_button_label: this.options.cancel_button_label
                ? this.options.cancel_button_label
                : i18n.gettext("Cancel"),
            send_button_label: this.options.send_button_label
                ? this.options.send_button_label
                : i18n.gettext("Send"),
            subject_label: this.options.subject_label
                ? this.options.subject_label
            ...
            reply_idea_title: reply_idea
                ? reply_idea.getShortTitleDisplayText(this.translationData)
                : "",
            ...
            collectionManager
                ...
                .then(function (discussion) {
                    ...
                    if (that.reply_message_model) {
                        routeUrl = that.reply_message_model.getRouterUrl({
                            relative: true,
                        });
                    }
                    var rolesMissingMessageForPermission = Ctx.getCurrentUser().getRolesMissingMessageForPermission(
                    ...
                    if ("reply_message_id" in that.options) {
                        messageString = i18n.gettext(
                            "Before you can reply to this message %s"
                        );
                    } else {
                        messageString = i18n.gettext(
                            "Before you can post a message %s"
                        );
                    }

                    warningMessage = i18n.sprintf(
                        messageString,
                        rolesMissingMessageForPermission
                    );
    ...
    onSendMessageButtonClick(ev) {
        ...
        var message_subject =
            message_subject_field.val() || this.options.default_subject;
        var reply_idea_id = null;
        var reply_message_id = null;
        var success_callback = null;
        var chosenTargetIdeaField = this.$el.find(
            ".messageSend-target input:checked"
        );
    ...
    */
        ...
        if (
            !message_subject &&
            (this.options.mandatory_subject_missing_msg ||
                (!reply_idea_id && !reply_message_id))
        ) {
            if (this.options.mandatory_subject_missing_msg) {
                ...
                alert(
                    i18n.gettext(
                        "You need to set a subject before you can send your message..."
                    )
                );
            }
        ...
        if (!message_body) {
            if (this.options.mandatory_body_missing_msg) {
                ...
                alert(
                    i18n.gettext(
                        "You need to type a message before you can send your message..."
                    )
                );
            }
        ...
        var subject_ls = message_subject
            ? new LangString.Model({
        ...
        this.model.save(null, {
            success: function (model, resp) {
                ...
                analytics.trackEvent(
                    analytics.events[
                        "MESSAGE_POSTED_ON_" + that.analytics_context
                    ]
                );

                Promise.resolve(that.attachmentsCollection.saveAll()).then(
                    function () {
                        ...
                        if (Ctx.getDiscussionId() && Ctx.getCurrentUserId()) {
                            Promise.join(
                                ...
                                function (
                                    ...
                                    var defaultActiveNotificationsDicussion = _.filter(
                                        notificationsDiscussion.models,
                                        function (model) {
                                            // keep only the list of notifications which become active when a user follows a discussion
                                            return (
                                                model.get("creation_origin") ===
                                                    "DISCUSSION_DEFAULT" &&
                                    ...
                                    var userActiveNotifications = _.filter(
                                        notificationsUser.models,
                                        function (model) {
                                            return (
                                                model.get("status") === "ACTIVE"
                                            );
                                    ...
                                    agent.fetch({
                                        success: function (model, resp) {
                                            ...
                                            if (
                                                (agent.get("post_count") ===
                                                    0 ||
                                                    agent.get("post_count") <
                                                        2) &&
                                                userActiveNotifications.length <
                                                    defaultActiveNotificationsDicussion.length
                                            ) {
                        ...
                        if (that.messageList) {
                            that.messageList
                                ...
                                .then(function () {
                                    if (
                                        _.isFunction(that.options.send_callback)
                                    ) {
                                        that.options.send_callback();
                                    }

                                    var current_idea = that.messageList
                                        .getGroupState()
                                        .get("currentIdea");
                                    ...
                                    if (
                                        ...
                                        that.messageList.showMessageById(
                                            model.id
                                        );
                                    }
                        ...
                        */
                                        ...
                                        if (that.messageList) {
                                            that.messageList.triggerMethod(
                                                "messageList:clearAllFilters"
                                            );
    ...
    */
        if (messageWasSaved) {
            analytics.trackEvent(
                analytics.events[
                    "LEAVE_NON_EMPTY_MESSAGE_WRITING_AREA_ON_" +
                        this.analytics_context
                ]
            );
        } else {
            analytics.trackEvent(
                analytics.events[
                    "LEAVE_EMPTY_MESSAGE_WRITING_AREA_ON_" +
                        this.analytics_context
                ]
            );
        }
     ...
     */
        if (
            (message_body.length > 0 && message_body.val().length > 0) ||
            (message_title.length > 0 && message_title.val().length > 0)
        ) {
            analytics.trackEvent(
                analytics.events[
                    "ENTER_NON_EMPTY_MESSAGE_WRITING_AREA_ON_" +
                        this.analytics_context
                ]
            );
        } else {
            analytics.trackEvent(
                analytics.events[
                    "ENTER_EMPTY_MESSAGE_WRITING_AREA_ON_" +
                        this.analytics_context
                ]
            );
        }
    ...
    savePartialMessage() {
        if (this.isRenderedAndNotYetDestroyed()) {
            ...
            if (
                ...
                MessagesInProgress.saveMessage(
                    this.msg_in_progress_ctx,
                    message_body.val(),
                    message_title.val()
                );
                return true;
    ...
    clearPartialMessage() {
        if (this.ui.messageBody.length > 0) this.ui.messageBody.val("");
        if (this.ui.messageSubject.length > 0) this.ui.messageSubject.val("");
