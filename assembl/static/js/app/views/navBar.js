...
class navBarLeft extends Marionette.View.extend({
    ...
    initialize(options) {
        this.isAdminDiscussion = Ctx.getCurrentUser().can(
                    ...
                    if (!that.isDestroyed()) {
                        ...
                        if (rootIdea) {
                            var confWidgets = new Widget.WidgetSubset([], {
                                parent: widgets,
                                context:
                                    Widget.Model.prototype
                                        .DISCUSSION_MENU_CONFIGURE_CTX,
                                idea: rootIdea,
                            });
                            if (confWidgets.length) {
                                var configuration = new WidgetLinks.WidgetLinkListView(
                                    { collection: confWidgets }
                                );
                                that.showChildView(
                                    "widgetMenuConfig",
                                    configuration
                            ...
                            this.$el
                                .find(".discussion-title-dropdown")
                                .addClass("hidden");
                        }
                    }
                }
    ...
    serializeData() {
        return {
            ...
            canAccessStatistics: Ctx.getCurrentUser().can(
                Permissions.DISC_STATS
            ),
            discussionSettings: "/" + Ctx.getDiscussionSlug() + "/edition",
            discussionPermissions:
                "/admin/permissions/discussion/" + Ctx.getDiscussionId(),
        };
    }
...
class navBarRight extends LoaderView.extend({
        ...
        if (Ctx.getDiscussionId() && Ctx.getCurrentUserId()) {
            ...
            collectionManager
                ...
                .then(function (localRoles) {
                    ...
                    if (localRoles) {
                        that.listenTo(localRoles, "remove add", function (
                            model
                        ) {
                            that.isUserSubscribedToDiscussion = localRoles.isUserSubscribedToDiscussion();
                            that.render();
    ...
    serializeData() {
        ...
        return {
            ...
            canSubscribeToDiscussion: Ctx.getCurrentUser().can(
                Permissions.SELF_REGISTER
            ),
            isAdminDiscussion: Ctx.getCurrentUser().can(
                Permissions.ADMIN_DISCUSSION
            ),
        };
    }
...
class navBar extends Marionette.View.extend({
    ...
    initialize() {
        ...
        this.listenTo(
            IdeaLoom.other_vent,
            "navBar:subscribeOnFirstPost",
            this.showPopInOnFirstPost
        );
        this.listenTo(
            IdeaLoom.other_vent,
            "navBar:joinDiscussion",
            this.joinDiscussion
        );
    }
    ...
    addGroup() {
        ...
        IdeaLoom.rootView.showChildView(
            "slider",
            new DefineGroupModal({ groupSpecsP: groupSpecsP })
        );
    }
    ...
    joinDiscussion(evt, popinType) {
        ...
        if (popinType == "first_post") {
            ...
            popinType == "first_login_after_auto_subscribe_to_notifications"
        ) {
            modalClassName = "generic-modal popin-wrapper modal-firstPost";
            modalTemplate = _.template(
                $("#tmpl-firstLoginAfterAutoSubscribeToNotifications").html()
            );
        }
        ...
        Promise.join(
            ...
            function (discussionNotifications, localRoles) {
                ...
                model.notificationsToShow = _.filter(
                    discussionNotifications.models,
                    function (m) {
                        // keep only the list of notifications which become active when a user follows a discussion
                        return (
                            m.get("creation_origin") === "DISCUSSION_DEFAULT" &&
                            m.get("status") === "ACTIVE"
                        );
                    }
                );
                ...
                class Modal extends Backbone.Modal.extend({
                    ...
                    initialize() {
                        ...
                        analytics.changeCurrentPage(
                            analytics.pages.NOTIFICATION
                        );
                    }
                    ...
                    submit(ev) {
                        ...
                        if (
                            ...
                            var LocalRolesUser = new RoleModels.localRoleModel({
                                ...
                                discussion:
                                    "local:Discussion/" + Ctx.getDiscussionId(),
                            });
                            LocalRolesUser.save(null, {
                                success: function (model, resp) {
                                    var analytics = Analytics.getInstance();
                                    analytics.trackEvent(
                                        analytics.events.JOIN_DISCUSSION
                                    );

                                    // TODO: Is there a simpler way to do this? MAP
                                    self.getRegion(
                                        "navBarRight"
                                    ).currentView.ui.joinDiscussion.css(
                                        "visibility",
                                        "hidden"
                                    );
                                    self._store.removeItem(
                                        "needJoinDiscussion"
                                    );
                                    ...
                                    try {
                                        // we try to be a good Single Page Application and update user data without reloading the whole page
                                        Ctx.updateCurrentUser();
                                    } catch (e) {
                                        // but if it does not work, we reload the page
                                        console.log(
                                            "Error while reloading user data: " +
                                                e.message
                                        );
                                        location.reload();
                                    }
                                ...
                                error: function (model, resp) {
                                    console.error(
                                        "ERROR: joinDiscussion->subscription",
                                        resp
                                    );
                                },
                            });
                    ...
                    cancel() {
                        ...
                        var analytics = Analytics.getInstance();
                        analytics.trackEvent(
                            analytics.events.JOIN_DISCUSSION_REFUSED
                        );
                        analytics.changeCurrentPage(this.returningPage, {
                            default: true,
                        }); //if page is null, go back to / page
                    }
                }
    ...
    showPopInOnFirstLoginAfterAutoSubscribeToNotifications() {
        if (
            typeof first_login_after_auto_subscribe_to_notifications !=
                "undefined" &&
