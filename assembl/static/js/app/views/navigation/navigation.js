...
class NavigationView extends BasePanel.extend({
    ...
    onAttach() {
        ...
        var boundSetSideBarHeight = function () {
            return that.setSideBarHeight();
        };
        ...
        collectionManager
            ...
            .then(function (discussion) {
                ...
                if (navigationItemCollections.length > 0) {
                    ...
                    that.visualizationItems.reset(
                        navigationItemCollections[0].models
                    );
                    that.num_items += 1;
            ...
            .then(function (allMessageStructureCollection) {
                if (allMessageStructureCollection.getLastSynthesisPost()) {
                    ...
                    if (
                        that
                            .getContainingGroup()
                            .model.get("navigationState") !== "synthesis"
                    ) {
                ...
                var messageArrivedCallback = _.bind(function () {
                    console.log(
                        "navigation::onAttach::messageArrivedCallback()"
                    );
                    if (this.getLastSynthesisPost()) {
                        console.log(
                            "we now have a synthesis, show synthesis tab"
                        );
                        that.ui.synthesis_tab.show();
                    }
    ...
    _toggleMenuByEvent(evt) {
        if ($(evt.target).hasClass("js_minimizePanel")) return;
    ...
    setSideBarHeight() {
        var that = this;
        var debouncedFunction = _.debounce(
            function () {
        ...
        switch (view) {
            case "about":
                ...
                if (origin !== null) {
                    analytics.trackEvent(
                        analytics.events.NAVIGATION_OPEN_CONTEXT_SECTION
                    );
                    analytics.changeCurrentPage(
                        analytics.pages.SIMPLEUI_CONTEXT_SECTION
                    );
                }
            ...
            case "debate":
                ...
                if (origin !== null) {
                    ...
                    analytics.changeCurrentPage(
                        analytics.pages.SIMPLEUI_DEBATE_SECTION
                    );
                }
            ...
            case "synthesis":
                ...
                if (origin !== null) {
                    analytics.trackEvent(
                        analytics.events.NAVIGATION_OPEN_SYNTHESES_SECTION
                    );
                    analytics.changeCurrentPage(
                        analytics.pages.SIMPLEUI_SYNTHESES_SECTION
                    );
                }
                this.getContainingGroup().NavigationResetSynthesisMessagesState(
                    synthesisInNavigationPanel
                );
                break;
            case "visualizations":
                ...
                if (origin !== null) {
                    analytics.trackEvent(
                        analytics.events.NAVIGATION_OPEN_VISUALIZATIONS_SECTION
                    );
                    analytics.changeCurrentPage(
                        analytics.pages.SIMPLEUI_VISUALIZATIONS_SECTION
                    );
                }
                break;
            default:
    ...
    initVar() {
        ...
        var _headerGroup = $(".groupHeader").first().hasClass("editable")
            ? this.group_editable_header_height
            : this.group_header_height;
        var _sideBarHeight = _window - _header - _headerGroup;
        var that = this;

        if (this.$el && this.$el.parent() && this.$el.parent().height()) {
            this._accordionContentHeight =
                _sideBarHeight - _li - marginHeightLi;
        } else {
            // fallback: set an initial estimation
            this._accordionContentHeight =
                _sideBarHeight - _li - marginHeightLi;

            if (++this._accordionHeightTries < 10) {
