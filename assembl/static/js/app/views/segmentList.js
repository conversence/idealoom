...
class SegmentView extends Marionette.View.extend({
    ...
    serializeData() {
        ...
        var harvester = this.model.getCreatorFromUsersCollection(
            this.allUsersCollection
        );
        ...
        if (idPost) {
            ...
            if (post) {
                this.postCreator = this.allUsersCollection.get(
                    post.get("idCreator")
                );
            }
        }
...
class SegmentListView extends Marionette.CollectionView.extend({
    ...
    initialize(options) {
        if (
            this.collection.name == "IdeaSegmentList" &&
...
class IdeaSegmentListSubset extends Backbone.Subset.extend({
    ...
    sieve(extract) {
        return (
            extract.linkedToIdea(this.ideaId) &&
            (!this.importantOnly || extract.get("important"))
        );
    }
...
class SegmentListPanel extends BasePanel.extend({
        ...
        collectionManager
            ...
            .then(function (allExtractsCollection) {
                if (!that.isDestroyed()) {
                    ...
                    that.listenTo(allExtractsCollection, "invalid", function (
                        model,
                        error
                    ) {
                        alert(error);
    ...
    resetTitle() {
        if (this.clipboard) {
            ...
            this.getPanelWrapper().resetTitle(
                "<i class='icon-clipboard'></i> " +
                    i18n.gettext("Clipboard") +
                    " (" +
                    numExtracts +
                    ")"
            );
            ...
            this.getPanelWrapper().resetTitle(
                i18n.gettext("Clipboard") + " (" + i18n.gettext("empty") + ")"
            );
        }
    }
    ...
    onDragOver(ev) {
        ...
        if (
            ...
            _.indexOf(ev.dataTransfer.types, "text/plain") > -1
        ) {
    ...
    onClearButtonClick(ev) {
        ...
        var ok = confirm(
            i18n.gettext(
                "Are you sure you want to empty your entire clipboard?"
            )
        );
