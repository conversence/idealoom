    ...
    postRender() {
        ...
        Promise.join(
            ...
            (allSynthesisCollection, translationData) => {
                ...
                if (!synthesis) {
                    throw Error(
                        "BUG: Could not get synthesis after post. Maybe too early."
                    );
                }

                const subject = synthesis
                    .get("subject")
                    .bestValue(translationData);
                that.$(".message-subject").html(subject);
                if (that.viewStyle == that.availableMessageViewStyles.PREVIEW) {
                    //Strip HTML from preview
                    //bodyFormat = "text/plain";

                    const introduction = synthesis
                        .get("introduction")
                        .bestValue(translationData);
                    const body = MessageView.prototype.generateBodyPreview(
                        introduction
                    );
