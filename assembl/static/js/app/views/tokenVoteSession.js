...
var transitionAnimation = function (el, el2, duration) {
  ...
  */
    ...
    if (el.css("display") == "none") {
        // DOM elements which have the "display: none" CSS property are excluded from the rendering tree and thus have a position that is undefined.
...
class RemainingCategoryTokensView extends View.extend({
    ...
    initialize(options) {
        ...
        this.myVotesCollection = this.options.myVotesCollection;
        this.listenTo(
            this.myVotesCollection,
            "change:category:" + this.model.getId(),
            this.render
        ); // this is a category-level refresh, to refresh only the ones which have changed (avoids animation glitches triggered by allocation clicks on exclusive categories icons)
    ...
    onRender() {
        ...
        var customTokenImagePromise = customTokenImageURL
            ? getSVGElementByURLPromise(customTokenImageURL)
            : $.Deferred().resolve();
        var customEmptyTokenImageURL = category.get("image_empty");
        var color = category.get("color");
        var customEmptyTokenImagePromise = customEmptyTokenImageURL
            ? getSVGElementByURLPromise(customEmptyTokenImageURL)
            : $.Deferred().resolve();
        ...
        var categoryNameBestTranslation = categoryNameLangString
            .bestWithErrors(that.languagePreferences, false)
            .entry.value();
        ...
        var s = i18n.sprintf(
            i18n.ngettext(
                ...
                "<span class='available-tokens-number'>%d</span> remaining %s tokens",
                data["remaining_tokens"]
            ),
            data["remaining_tokens"],
            categoryNameBestTranslation
        );
        el.html(s);
        ...
        $.when(customTokenImagePromise, customEmptyTokenImagePromise).then(
            function (fullToken, emptyToken) {
...
class TokenCategoryAllocationView extends View.extend({
    ...
    initialize(options) {
        ...
        this.myVotesCollection = this.options.myVotesCollection;
        this.showZeroIcon =
            "showZeroIcon" in this.options ? this.options.showZeroIcon : true;
        this.preventDefaultTokenClickBehaviour =
            "preventDefaultTokenClickBehaviour" in this.options
                ? this.options.preventDefaultTokenClickBehaviour
                : false;
        this.forceUnselectZero =
            "forceUnselectZero" in this.options
                ? this.options.forceUnselectZero
                : false;
        this.tokenSize =
            "tokenSize" in this.options ? this.options.tokenSize : null;
        this.listenTo(
            this.myVotesCollection,
            "change:category:" + this.model.getId(),
            this.render
        ); // force re-render of all other token allocation views of the same token category, so that only the right icons are clickable. We use this way instead of setting this.collection to this.myVotesCollection and adding a collectionEvents hash, because does it not seem to work (probably because the hash executes before initialize())

        var myVote = this.myVotesCollection.findWhere({
            idea: this.idea.get("@id"),
            token_category: this.model.get("@id"),
        });
        if (myVote) {
        ...
        if (
            !_.isNumber(maximum_per_idea) ||
            maximum_per_idea < 0 ||
            maximum_per_idea > 1000
        ) {
            maximum_per_idea = 10;
        }
        ...
        this.postData = {};
        var voting_urls =
            "voting_urls" in this.voteSpecification
                ? this.voteSpecification["voting_urls"]
                : null;
        ...
        this.customTokenImagePromise = this.customTokenImageURL
            ? getSVGElementByURLPromise(this.customTokenImageURL)
            : $.Deferred().resolve();

        this.customEmptyTokenImageURL = this.model.get("image_empty");
        this.customEmptyTokenImagePromise = this.customEmptyTokenImageURL
            ? getSVGElementByURLPromise(this.customEmptyTokenImageURL)
            : $.Deferred().resolve();
    ...
    onRender() {
        ...
        var renderClickableTokenIcon = function (
            number_of_tokens_represented_by_this_icon
        ) {
      ...
      */
            ...
            if (number_of_tokens_represented_by_this_icon == 0) {
                ...
                number_of_tokens_represented_by_this_icon <= that.currentValue
            ) {
                showAsSelected = true;
            }
            if (showAsSelected) {
            ...
            var tokenBagData = that.myVotesCollection.getTokenBagDataForCategory(
                that.model
            );
            var remaining_tokens = tokenBagData["remaining_tokens"];
            var userCanClickThisToken =
                remaining_tokens +
                    that.currentValue -
                    number_of_tokens_represented_by_this_icon >=
                0;
            if (userCanClickThisToken) {
                ...
                tokenContainer.attr(
                    "title",
                    i18n.gettext("You don't have enough tokens remaining.")
                );
            }
        ...
        var renderAllTokenIcons = function () {
            for (
                var i = container[0].childElementCount;
                i <= that.maximum_per_idea;
                ++i
            ) {
        ...
        $.when(
            that.customTokenImagePromise,
            that.customEmptyTokenImagePromise
        ).then(function (fullIcon, emptyIcon) {
            if (that.customTokenImageURL) {
    ...
    onTokenIconClick(number_of_tokens_represented_by_this_icon) {
        ...
        var tokenContainer = that.getTokenIconForValue(
            number_of_tokens_represented_by_this_icon
        );
        ...
        if (
            ...
            ((!that.forceUnselectZero &&
                tokenIconElement.hasClass("selected")) ||
                (that.forceUnselectZero &&
                    tokenIconElement.hasClass("not-selected")))
        ) {
            return;
        }

        console.log(
            "set " + number_of_tokens_represented_by_this_icon + " tokens"
        );
        ...
        if (that.currentValue < number_of_tokens_represented_by_this_icon) {
            // we are adding tokens to this idea
            for (
                var i = that.currentValue + 1;
                i <= number_of_tokens_represented_by_this_icon;
                ++i
            ) {
                var selector =
                    ".token-vote-session .token-bag-for-category." +
                    that.model.getCssClassFromId() +
                    " .available-tokens-icons .available";
                console.log("selector: ", selector);
                var theAvailableToken = $(selector).eq(
                    $(selector).length -
                        1 -
                        (number_of_tokens_represented_by_this_icon - i)
                );
                var theAllocatedToken = tokenContainer
                    .parent()
                    .children()
                    .eq(i);
                theAvailableToken[0].classList.add(
                    "animating-towards-not-available"
                );
                theAllocatedToken[0].classList.add(
                    "animating-towards-selected"
                );
                setTimeout(
                    endAnimationTowardsNotAvailable(theAvailableToken[0]),
                    animation_duration * 0.9
                );
                transitionAnimation(
                    theAvailableToken.find("svg").first(),
                    theAllocatedToken.find("svg").first(),
                    animation_duration
                );
            }
        } else {
            // we are removing tokens from this idea
            var initial_value = number_of_tokens_represented_by_this_icon;
            for (
                var i = that.currentValue;
                i > number_of_tokens_represented_by_this_icon;
                --i
            ) {
                var selector =
                    ".token-vote-session .token-bag-for-category." +
                    that.model.getCssClassFromId() +
                    " .available-tokens-icons .not-available";
                console.log("selector: ", selector);
                var theAvailableToken = $(selector).eq(
                    i - number_of_tokens_represented_by_this_icon - 1
                );
                var theAllocatedToken = tokenContainer
                    .parent()
                    .children()
                    .eq(i);
                theAvailableToken[0].classList.add(
                    "animating-towards-available"
                );
                theAllocatedToken[0].classList.add(
                    "animating-towards-not-selected"
                );
                setTimeout(
                    endAnimationTowardsAvailable(theAvailableToken[0]),
                    animation_duration * 0.9
                );
                transitionAnimation(
                    theAllocatedToken.find("svg").first(),
    ...
    */
        ...
        setTimeout(function () {
            ...
            if (previousVote) {
                previousVote.set({
                    value: number_of_tokens_represented_by_this_icon,
                });
                previousVote.save();
            } else {
            ...
            that.currentValue = number_of_tokens_represented_by_this_icon;
            if (
                number_of_tokens_represented_by_this_icon > 0 ||
                !that.forceUnselectZero
            ) {
                tokenIconElement[0].classList.add("selected");
            }
            container.removeClass("hover");
            that.myVotesCollection.trigger(
                "change:category:" + that.model.getId()
            ); // force re-render of all token allocation views of this same token category (so that the right icons are clickable)
...
class TokenCategoryExclusivePairCollectionView extends View.extend({
    ...
    onRender() {
        ...
        var negativeTokensView = new TokenCategoryAllocationView(
            childViewOptions
        );
        childViewOptions.model = positiveTokens;
        childViewOptions.showZeroIcon = false;
        var positiveTokensView = new TokenCategoryAllocationView(
            childViewOptions
        );
        ...
        positiveTokensView.on("token:click", function (clicked_value) {
            console.log(
                "positiveTokensView click clicked_value: ",
                clicked_value
            );
        ...
        negativeTokensView.on("token:click", function (clicked_value) {
            console.log(
                "negativeTokensView click clicked_value: ",
                clicked_value
            );
            var currentNegativeValue = negativeTokensView.getCurrentValue();
            var currentPositiveValue = positiveTokensView.getCurrentValue();
...
class TokenVoteItemView extends View.extend({
    ...
    serializeData() {
        return {
            ideaTitle:
                ...
                1 +
                ". " +
                this.model.getShortTitleDisplayText(
    ...
    onRender() {
        var that = this;
        var tokenCategories =
            "tokenCategories" in this.parent.options
                ? this.parent.options.tokenCategories
                : null;
        var voteSpecification =
            "voteSpecification" in this.parent.options
                ? this.parent.options.voteSpecification
                : null;
        var myVotesCollection =
            "myVotesCollection" in this.parent.options
                ? this.parent.options.myVotesCollection
                : null;
        var tokenSize =
            "tokenSize" in this.parent.options
                ? this.parent.options.tokenSize
                : null;
        var idea = that.model;
        ...
        if (tokenCategories) {
            ...
            if (
                tokenCategories.length == 2 &&
                voteSpecification &&
                "exclusive_categories" in voteSpecification &&
                voteSpecification.exclusive_categories
            ) {
...
class TokenVoteCollectionView extends View.extend({
    ...
    onRender() {
        this.showChildView(
            "votes",
            new TokenVoteCollectionViewBody({
...
class TokenVoteResultView extends View.extend({
    ...
    initialize(options) {
        ...
        this.categoryNumber = _.indexOf(
            this.categoryIndex,
            this.model.get("typename")
        );
    ...
    _getCategoryName(catName) {
        ...
        return categoryModel
            .get("name")
            .bestWithErrors(this.languagePreferences, false)
            .entry.value();
    }
    ...
    serializeData() {
        ...
        return {
            ideaTitle: this.model
                .get("objectConnectedTo")
                .getShortTitleDisplayText(this.userLanguagePreferences),
            categoryResult: this.results,
            showDescriptionButton:
                defn && !defn.isEmptyStripped(this.userLanguagePreferences),
    ...
    onRender() {
        ...
        var displayTooltip = function (num, total, category) {
            ...
            return i18n.sprintf(
                i18n.ngettext(
                    '%d token "%s" was voted on this idea, out of a total %d "%s" tokens voted ',
                    '%d tokens "%s" were voted on this idea, out of a total %d "%s" tokens voted',
                    num
                ),
                num,
                category,
        ...
        results
            ...
            .attr("title", function (r) {
                return displayTooltip(r.sum, r.totalTokens, r.categoryName);
            })
    ...
    onSeeDescriptionClick(ev) {
        ...
        if (this.shownDescription === true) {
            ...
            this.ui.descriptionRegion.html(
                defn ? defn.bestValue(this.userLanguagePreferences) : ""
            );
        }
    }
}
...
class TokenVoteResultCollectionView extends View.extend({
    ...
    initialize(options) {
        ...
        this.voteSpecification = options.voteSpecification;
        this.languagePreferences = options.languagePreferences;
        this.sortAscending = _.map(this.categoryIndex, function () {
            return false;
        });
        this.voteResults.sortSpecName = this.categoryIndex[
            this.sortOnCategoryNum
        ];
    ...
    onRender() {
        ...
        this.showChildView(
            "voteResults",
            new TokenVoteResultCollectionViewBody({
    ...
    serializeData() {
        ...
        categories = _.map(categories, function (cat) {
            ...
            var best = categoryLangString.bestWithErrors(
                that.languagePreferences,
                false
            );
...
class TokenResultView extends View.extend({
    ...
    initialize(options) {
        ...
        cm.getAllIdeasCollectionPromise().then(function (ideas) {
            that.ideas = ideas;
            ...
            return cm
                ...
                .then(function () {
                    that.voteResults = new Widget.VoteResultCollection({
                        widgetModel: that.model,
                        parse: true,
                    });
    ...
    serializeData() {
        ...
        var questionTitle =
            "question_title" in questionItem ? questionItem.question_title : "";
        var questionDescription =
            "question_description" in questionItem
                ? questionItem.question_description
                : "";
        var startDate = Ctx.getNiceDate(
            this.model.get("start_date"),
            true,
            true,
            false
        );
        var endDate = Ctx.getNiceDate(
        ...
        if (!this.isReady) {
            ...
            statement += i18n.sprintf(
                i18n.ngettext(
                    ...
                    "%d participants voted between %s and %s. The number of tokens available for voting per participant:",
                    that.numVoters
                ),
                that.numVoters,
                startDate,
            ...
            _.each(data, function (category) {
                ...
                statement += i18n.sprintf(
                    i18n.ngettext(
                        '<strong>%d</strong> token of type "%s"',
                        '<strong>%d</strong> tokens of type "%s"',
                        category.number
                    ),
                    category.number,
                    category.name
...
class TokenVoteSessionModal extends Backbone.Modal.extend({
    ...
    initialize(options) {
        ...
        if (voteSpecifications && voteSpecifications.length > 0) {
            that.tokenVoteSpecification = _.findWhere(voteSpecifications, {
                "@type": "TokenVoteSpecification",
            });
            if (that.tokenVoteSpecification) {
                if ("token_categories" in that.tokenVoteSpecification) {
                    if (
                        _.isArray(that.tokenVoteSpecification.token_categories)
                    ) {
                        console.log(
                            "going to parse tokenVoteSpecification.token_categories and convert it into a TokenCategorySpecificationCollection"
                        );
                        that.tokenCategories = new Widget.TokenCategorySpecificationCollection(
                            that.tokenVoteSpecification.token_categories,
                            { parse: true }
                        );
                    } else if (
                        _.isObject(that.tokenVoteSpecification.token_categories)
                    ) {
                        console.log(
                            "wtf, tokenVoteSpecification.token_categories was already parsed and converted into a TokenCategorySpecificationCollection"
                        );
                        that.tokenCategories =
                            that.tokenVoteSpecification.token_categories;
                    }
                }
            }
        ...
        var myVotes =
            "my_votes" in that.tokenVoteSpecification
                ? that.tokenVoteSpecification.my_votes
                : null; // TODO: maybe we should dynamically load user's votes on each time the user opens the popin, instead of relying on potentially outdated user vote data
        that.myVotesCollection = new Widget.TokenIdeaVoteCollection(myVotes);

        // This URL needs the idea id in the JSON payload
        var genericVotingUrl =
            "voting_url" in that.tokenVoteSpecification
                ? that.tokenVoteSpecification.voting_url
                : null; // for example: http://localhost:6543/data/Conversation/6/widgets/90/vote_specifications/22/votes
        ...
        Promise.join(
            ...
            function (userLanguagePreferences, allIdeasCollection) {
                ...
                var orderedVotableIdeas = that.votableIdeasCollection.sortBy(
                    function (idea) {
                        return idea.id;
                    }
                ); // /!\ with this, "local:Idea/257" < "local:Idea/36"
                ...
                if (n < 100) {
                    ...
                    if (u) {
                        ...
                        var permutation = nthPermutation(
                            orderedVotableIdeas,
                            permutationIndex,
                            orderedVotableIdeas.length
                        );
                        console.log("permutation: ", permutation);
                    }
                }

                var tokenVoteSpecificationModel = new Widget.TokenVoteSpecificationModel(
                    that.tokenVoteSpecification,
                    { parse: true }
                );
                console.log(
                    "tokenVoteSpecificationModel: ",
                    tokenVoteSpecificationModel
                );
                var maximumTokensPerRow = that.computeMaximumTokensPerRow(
                    tokenVoteSpecificationModel
                );
                ...
                var collectionView = new TokenVoteCollectionView({
                    ...
                    viewComparator: function (idea) {
                        return _.findIndex(permutation, function (idea2) {
    ...
    onShow() {
        ...
        that.availableTokensPositionTop = that
            .$(".available-tokens")
            .position().top;
        that.$(".available-tokens").width(that.$(".popin-body").width());
        var popinHeaderTotalHeight = that.$(".popin-header").outerHeight();
        that.$(".available-tokens").css(
            "top",
            popinHeaderTotalHeight + (popinHeaderTotalHeight > 0 ? "px" : "")
        );
    ...
    myOnScroll() {
        this.$(".available-tokens").toggleClass(
            "fixed",
            this.$(".popin-body").scrollTop() > this.availableTokensPositionTop
        );
    }
    ...
    serializeData() {
        ...
        return {
            ...
            question_title:
                "question_title" in question_item
                    ? question_item.question_title
                    : "",
            question_description:
                "question_description" in question_item
                    ? question_item.question_description
                    : "",
            available_tokens_info: i18n.gettext(
                "Split your tokens among the ideas of your choice. By default, your vote is neutral per project."
            ),
  ...
  */
    computeMaximumTokensPerRow(tokenVoteSpecification) {
        ...
        if (
            tokenCategories.length == 2 &&
            tokenVoteSpecification.get("exclusive_categories") == true
        ) {
            maximum_tokens_per_idea_row = tokenCategories.reduce(function (
                memo,
                category
            ) {
        ...
        var maximum_tokens_in_a_bag =
            tokenCategories.max("total_number").get("total_number") || 0;
        maximum_tokens_per_row =
            Math.max(maximum_tokens_per_idea_row, maximum_tokens_in_a_bag) || 0;
